---
name: orion-ui-blueprint-framework
...
- 创建、命名、保存、移动或整理 UI 资产时，配合 `../orion-asset-management/SKILL.md`。
- 普通 UMG / CommonUI / UIExtension 工作继续配合 `../orion-umg/SKILL.md`；本 Skill 负责当前 `Content/UI` 资产族的具体约定。
- UI 输入、back/click、Action Domain、key brush、手柄图标，配合 `../unreal-commoninput/SKILL.md`。
- Settings UI 和可配置项，配合 `../orion-game-settings-framework/SKILL.md`。
- LoadingScreen Widget 外观和加载生命周期，配合 `../orion-loading-screen-framework/SKILL.md`。
- Online/Friends/Session UI，配合 `../orion-online-steam-framework/SKILL.md`。
- Interaction/Indicator/key prompt UI，配合 `../orion-inventory-interaction-framework/SKILL.md`。
- Feature 或 Experience 激活时添加 UI，配合 `../orion-gamefeatures/SKILL.md`、`../orion-gamemode-experience-framework/SKILL.md` 和 `../orion-mcp-project-toolsets/SKILL.md`。
