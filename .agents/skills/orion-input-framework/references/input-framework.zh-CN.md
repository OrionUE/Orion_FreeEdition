...
- 玩法插件仍应遵守 GameFeatures 内容组织规则；创建正式资产前先读取 `orion-asset-management`。

如果输入资产用于 UI 按键提示或 CommonInput controller data，还要检查 `Content/UI` 与平台 controller data 目录规则。

## 玩家改键和按键设置页

设置 UI、Apply/Cancel、VisualData 和保存行为先读取 `orion-game-settings-framework`；本节只记录输入系统如何向设置页提供可改键数据。
...
如果可用 MCP 修改 Experience 或 ActionSet，优先读取 `orion-mcp-project-toolsets`。输入相关工具入口：
...
创建完整关卡玩法输入闭环时，先读取 `../../orion-gamemode-experience-framework/SKILL.md`，确认 Experience、PawnData、AbilitySet、InputConfig、InputAction、InputTag 和 IMC 的关系。
