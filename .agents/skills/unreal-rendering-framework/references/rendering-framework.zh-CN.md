...
| `Config/<Platform>/<Platform>Engine.ini` | 平台渲染和 RHI 覆盖 | Windows 中启用 RayTracing、PSO Precaching、ShaderPipelineCache、FSR3 DX12、sync、VT pool 等。 |
...
| `Config/<Platform>/<Platform>Game.ini` | 平台 Game 配置 | 移动平台可覆盖 `OrionPlatformSpecificRenderingSettings` 的 frame pacing。 |
