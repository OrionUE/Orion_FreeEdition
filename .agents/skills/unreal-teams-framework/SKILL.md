...
- 判断 GameCore 只读边界、TeamSubsystem 的 `UCoreWorldSubsystem` 基类、GameplayTagStack 或核心系统扩展点时，配合 `../unreal-gamecore-framework/SKILL.md` 和 `../unreal-system-framework/SKILL.md`。
- 队伍创建、玩法队伍数量、TeamCreationComponent 挂到 GameState、Experience/ActionSet 启用队伍规则时，配合 `../unreal-gamefeatures/SKILL.md` 和 `../unreal-gamemode-experience-framework/SKILL.md`。
- 玩家所属队伍、玩家账户决定队伍、PlayerController/GameState 组件挂载、玩家进入/退出重新分队时，配合 `../unreal-player-framework/SKILL.md`。
- 角色出生点按队伍分组、CharacterSpawner、PlayerStart、PawnData、角色蓝图和出生管理时，配合 `../unreal-character-pawn-framework/SKILL.md`。
- 伤害、锁定、Buff/Debuff、友军伤害、目标过滤或 GA 里读取队伍态度时，配合 `../unreal-gameplayabilities/SKILL.md`。
- 队伍变更、击杀、占点、UI 提示或玩法统计需要跨系统通知时，配合 `../unreal-gameplay-message-router/SKILL.md`。
- TeamInfo、TeamTags、TeamDisplayAsset、公开/私有队伍状态、队伍 owner-only 数据或按队伍可见性涉及网络复制/RPC 时，配合 `../unreal-network-replication-framework/SKILL.md`。
- 创建 TeamDisplayAsset、TeamInfo 蓝图、队伍 UI、GameFeature 内容或 Spawner 资产时，配合 `../unreal-asset-management/SKILL.md` 和 `../unreal-mcp-project-toolsets/SKILL.md`。
