...
`clean-engine-saved.ps1` 用于清理当前用户的 Unreal Engine Saved 目录，默认目标是 `%LOCALAPPDATA%\UnrealEngine\<EngineVersion>\Saved`。它不处理项目 `Saved/`，也不默认处理安装版引擎目录。
...
```powershell
& .agents\skills\orion-project-cleanup\scripts\clean-engine-saved.ps1 -EngineVersion 5.8 -Scope ContentBrowser
& .agents\skills\orion-project-cleanup\scripts\clean-engine-saved.ps1 -EngineVersion 5.8 -Scope WindowsEditorConfig
& .agents\skills\orion-project-cleanup\scripts\clean-engine-saved.ps1 -EngineVersion 5.8 -Scope All
```

确认输出的 `Delete candidates` 后执行：

```powershell
& .agents\skills\orion-project-cleanup\scripts\clean-engine-saved.ps1 -EngineVersion 5.8 -Scope All -Apply
```

安全规则：

- `-Apply` 前如果检测到 `UnrealEditor*` 进程正在运行，会拒绝执行，避免编辑器退出时把旧设置重新写回。
- 默认只允许清理 `%LOCALAPPDATA%\UnrealEngine\...\Saved` 下的目录。
- 安装版引擎的 `<EngineRoot>/Engine/Saved` 必须显式使用 `-Target InstalledEngine -EngineRoot <EngineRoot> -AllowInstalledEngineSaved`；不要把这个开关用于源码、配置或二进制目录。
- 自定义 `-SavedRoot` 默认必须仍在 `%LOCALAPPDATA%\UnrealEngine` 下；其他路径需要 `-AllowCustomSavedRoot`。

范围含义：

| Scope | 目标 |
| --- | --- |
| `All` | 删除 Saved 根目录下的所有子项，但不删除 Saved 根目录本身。 |
| `Config` | 删除 `Saved/Config`。 |
| `WindowsEditorConfig` | 删除 `Saved/Config/WindowsEditor`。 |
| `ContentBrowser` | 删除 `Saved/Config/WindowsEditor/EditorSettings.ini`，用于重置 Content Browser 全局用户设置。 |
| `Logs` | 删除 `Saved/Logs`。 |
| `Crashes` | 删除 `Saved/Crashes`。 |
...
```powershell
& .agents\skills\orion-project-cleanup\scripts\clean-ignored-project.ps1 -ListLimit 1000
```
