---
name: orion-build-platform-icons
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .agents\skills\orion-build-platform-icons\scripts\update-build-platform-icons.ps1 -SourceLogo <SOURCE_LOGO>
```
...
- 只讨论项目根目录 `Build` 文件夹职责、`Config` / `Content` / `Source` 分层或模块落点时，配合 `../orion-framework-architecture/SKILL.md`。
- 修改或验证打包结果、Steam Profile、UAT、staging、PSO、movie、本地化时，配合 `../orion-packaging/SKILL.md`；应用图标生成仍回到本 Skill。
- 创建、移动、导入普通 UE Content 资产时，配合 `../orion-asset-management/SKILL.md`；`Build/<Platform>` 应用图标不是普通 Content 资产。
