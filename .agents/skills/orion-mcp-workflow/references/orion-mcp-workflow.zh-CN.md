...
- 通用 HTTP MCP 客户端模板放在 `.agents/MCP/unreal-mcp-http.json`；不要再使用项目根目录 `MCP/` 作为模板目录。
...
`describe_toolset` 的 text 是 JSON schema 字符串，需要二次 `ConvertFrom-Json`。聚合入口下工具名通常仍显示为完整名，例如 `editor_toolset.toolsets.asset.AssetTools.find_assets`，但传给 `call_tool.tool_name` 时使用短名 `find_assets`。完整工具名只在 `ProgrammaticToolset.execute_tool_script` 里的 `execute_tool("editor_toolset.toolsets.asset.AssetTools.find_assets", ...)` 使用。
...
`ObjectTools.list_properties` 返回的是 JSON schema 字符串，不是逐行属性列表；需要 `json.loads(raw)` 后取 schema 的 key。它可能很大，尤其是 `CameraComponent.postProcessSettings`。不要为了筛选少量字段直接 dump 全量输出；已知字段可在确认属性存在后用 `ObjectTools.get_properties` 精确读取。

`ObjectTools.get_properties` 只要请求列表里有任一属性在当前对象类上不存在，就会整次失败并返回 warning。跨 Actor、Component、MaterialInstance 等类排查时，先按类解析 `list_properties`，或逐个属性读取并捕获异常。

`ObjectTools.get_properties` 不支持 `settings.autoExposureMethod` 这类嵌套路径。读取 PostProcessVolume 时先读取完整 `settings` 结构，再在脚本里提取 `autoExposureMethod`、`autoExposureBias`、`dynamicGlobalIlluminationMethod` 等子字段。
