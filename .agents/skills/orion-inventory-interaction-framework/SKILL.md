---
name: orion-inventory-interaction-framework
...
- 创建或整理可交互物品、GA、WBP、InputAction、AbilitySet、PawnData、Experience 或 GameFeature 内容资产时，配合 `../orion-asset-management/SKILL.md`。
...
- 交互输入、`InputTag.Ability.Interact`、`IA_Interact`、InputConfig、IMC 或改键可见性时，配合 `../orion-input-framework/SKILL.md` 和 `../unreal-enhancedinput/SKILL.md`。
- 交互提示 UI、Indicator widget、CommonActionWidget、按键图标或 CommonUI/UMG 布局时，配合 `../orion-umg/SKILL.md`、`../unreal-commoninput/SKILL.md`。
- 修改 `Content/UI/Interaction` 的交互提示 Widget、`UUI_InteractionWithKeyBrush` 蓝图或 Indicator UI 表现时，同时读取 `../orion-ui-blueprint-framework/SKILL.md`。
- 交互组件、IndicatorManager、输入、AbilitySet 或物品玩法随 Experience/GameFeature 启停时，配合 `../orion-gamefeatures/SKILL.md` 和 `../orion-gamemode-experience-framework/SKILL.md`。
- 世界物品、库存组件、ItemInstance、Container、RPC、replicated subobject、拾取/掉落/装备同步或多人交互 bug 时，配合 `../orion-network-replication-framework/SKILL.md`。
- 交互结果需要跨 UI、任务、提示、统计或系统解耦广播时，配合 `../orion-gameplay-message-router/SKILL.md`。
