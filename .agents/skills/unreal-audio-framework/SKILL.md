...
- 创建、移动、命名、导入或整理音频资产时，先配合 `../unreal-asset-management/SKILL.md`。
- 判断代码落点、模块职责、插件职责、Target 或构建变体时，配合 `../unreal-framework-architecture/SKILL.md`。
- 触碰 `UCoreAudioSettings`、GameCore 音频配置或核心基座边界时，配合 `../unreal-gamecore-framework/SKILL.md`。
- 查真实类、函数、变量、模块依赖、引擎 AudioMixer/AudioModulation/MetaSound 源码时，配合 `../unreal-source-code-navigator/SKILL.md`。
- 音频设置页、`GameSettingRegistry_Audio`、音量/HDR/设备用户设置保存或 CommonUI 设置界面，先配合 `../unreal-game-settings-framework/SKILL.md`；只改 Widget 外观时再配合 `../unreal-umg/SKILL.md`。
- 加载屏显示/隐藏驱动 `LoadingScreenControlBusMix`、CommonLoadingScreen visibility delegate 或加载屏媒体音频时，配合 `../unreal-loading-screen-framework/SKILL.md`。
- `OrionVoiceChat` 的语音包传输、频道同步、麦克风状态、Server/Client/NetMulticast RPC 或多人语音权限/范围判断，配合 `../unreal-network-replication-framework/SKILL.md`。
- 语音聊天 Ability、输入触发或 GameFeature 授予语音能力时，按需要配合 `../unreal-gameplayabilities/SKILL.md`、`../unreal-enhancedinput/SKILL.md` 和 `../unreal-gamefeatures/SKILL.md`。
