...
$SettingsJson = @{
	...
	BiasIncrement = $BiasIncrement
	MaxNewBias = $MaxNewBias
	Limit = $Limit
	SkipUI = [bool]$SkipUI
	Apply = [bool]$Apply
...
def run():
	...
	for folder in SETTINGS["Folders"]:
		for asset_path in find_assets(folder):
			...
			if not property_map.get("lodBias"):
				return {
					"apply": bool(SETTINGS["Apply"]),
					"error": "Texture LODBias property was not found through ObjectTools.list_properties."
				}
			...
			if bool(SETTINGS["SkipUI"]) and "UI" in lod_group:
				...
				continue
			old_bias = int(props[property_map["lodBias"]])
			new_bias = min(old_bias + int(SETTINGS["BiasIncrement"]), int(SETTINGS["MaxNewBias"]))
			...
			row = {
				...
				"newLODBias": new_bias,
				"lodGroup": lod_group,
				"expectedEffect": "Raises cooked/runtime mip bias by one level; top displayed/cooked mip is approximately half width and half height."
			}
			planned.append(row)
			if bool(SETTINGS["Apply"]):
				if not can_edit_asset(asset_path):
					...
					continue
				if set_properties(asset_path, {property_map["lodBias"]: new_bias}):
	...
	return {
		...
		"note": "Default mode is dry-run. Use -Apply to set LODBias and save assets."
	}
"@
