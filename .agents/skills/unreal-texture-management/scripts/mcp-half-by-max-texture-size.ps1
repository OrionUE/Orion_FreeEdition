...
$SettingsJson = @{
	...
	Limit = $Limit
	RoundToPowerOfTwo = [bool]$RoundToPowerOfTwo
	SkipUI = [bool]$SkipUI
	Apply = [bool]$Apply
...
def target_size(long_edge):
	raw_target = max(int(long_edge / 2), int(SETTINGS["MinimumTarget"]))
	if bool(SETTINGS["RoundToPowerOfTwo"]):
		return max(previous_power_of_two(raw_target), int(SETTINGS["MinimumTarget"]))
	return raw_target
...
def run():
	...
	for folder in SETTINGS["Folders"]:
		for asset_path in find_assets(folder):
			...
			if not property_map.get("maxTextureSize"):
				return {
					"apply": bool(SETTINGS["Apply"]),
					"error": "Texture MaxTextureSize property was not found through ObjectTools.list_properties."
				}
			...
			if bool(SETTINGS["SkipUI"]) and "UI" in lod_group:
				...
				continue
			old_max = int(props[property_map["maxTextureSize"]])
			new_max = target_size(long_edge)
			...
			row = {
				...
				"newMaxTextureSize": new_max,
				"lodGroup": lod_group,
				"expectedEffect": "Caps the top cooked mip to about half of the source long edge."
			}
			planned.append(row)
			if bool(SETTINGS["Apply"]):
				if not can_edit_asset(asset_path):
					...
					continue
				if set_properties(asset_path, {property_map["maxTextureSize"]: new_max}):
	...
	return {
		"apply": bool(SETTINGS["Apply"]),
		"method": "MaxTextureSize = current long edge / 2",
