...
5. 项目 `OrionProjectToolsets.OrionExperienceToolset` / `OrionProjectToolsets.OrionUMGToolset`
...
tools/list
tools/call name=list_toolsets
tools/call name=describe_toolset
tools/call name=load_toolset
tools/list
tools/call name=<FullToolName>
```
...
当前实测初始 `tools/list` 不含 `call_tool`。
...
当前结论：不要删除 `Plugins/MCP/OrionProjectToolsets`，但保持瘦身状态。
...
- 删除整个插件会破坏现有 Experience/GameFeature/UMG 自动化 Skill 和验证流程。
