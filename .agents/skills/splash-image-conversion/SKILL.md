...
本 Skill 用于把源图居中裁剪到目标比例，再缩放并导出 `.png` / `.bmp`。它只处理图片文件转换；正式 Unreal 启动图目录、固定文件名和覆盖决策仍先遵守 `../unreal-asset-management/SKILL.md` 与 `../unreal-umg/SKILL.md`。

## 工作流

1. 先判断用户只是要图片转换，还是要替换项目启动 Splash。
2. 如果要替换启动图，先读取 `../unreal-asset-management/SKILL.md`；正式输出使用 `Content/Splash/Splash.png`、`Content/Splash/Splash.bmp`、`Content/Splash/EdSplash.png`、`Content/Splash/EdSplash.bmp` 的固定命名。
