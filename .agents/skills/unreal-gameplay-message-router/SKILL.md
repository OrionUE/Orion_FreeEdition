---
name: unreal-gameplay-message-router
...
- 判断消息发送代码应落在 GameCore、宿主 Game 模块、GameFeature 插件、UI 模块还是 BBL 时，配合 `../unreal-framework-architecture/SKILL.md` 和 `../unreal-gamecore-framework/SKILL.md`；GameCore 默认只读。
...
- 队伍变更、队伍比分、占点、击杀归属、TeamDisplayAsset 刷新、友军伤害提示或基于团队的 UI 通知需要发消息时，配合 `../unreal-teams-framework/SKILL.md`。
- GameFeature 玩法插件之间、Feature Action 激活/反激活、玩法独立 UI/系统需要解耦通信时，配合 `../unreal-gamefeatures/SKILL.md`。
- 消息跨网络、server-to-all-clients、server-to-one-client、replicated state、owner-only 行为或 MessageRouter 网络桥接时，配合 `../unreal-network-replication-framework/SKILL.md`；需要 actor relevancy/RepGraph placement 再配合 `../unreal-replicationgraph/SKILL.md`。
- UI toast、击杀提示、错误提示、CommonUI Widget 监听消息时，配合 `../unreal-umg/SKILL.md` 和 `../unreal-commoninput/SKILL.md`。
- 新增 GameplayTag、tag table、Blueprint 资产、Widget 资产或 GameFeature 内容目录时，配合 `../unreal-asset-management/SKILL.md` 和项目配置/GameplayTags 规则。
