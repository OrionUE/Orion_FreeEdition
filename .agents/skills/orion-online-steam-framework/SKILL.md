---
name: orion-online-steam-framework
...
- 判断代码落在 GameCore、OrionGame、GameUI、BBL、OrionSteam 插件还是 GameFeature 插件时，配合 `../orion-framework-architecture/SKILL.md`。
- 触碰 `UCoreSessionComponent`、`UOrionGameSessionComponent`、Experience、GameState 组件或 GameCore 只读边界时，配合 `../orion-gamecore-framework/SKILL.md`。
- 触碰 `FOrionPlayerID`、玩家 Steam ID、玩家头像/昵称、好友条目、玩家账户或玩家信息组件时，配合 `../orion-player-framework/SKILL.md`。
- GameFeature 挂载 Session 组件、创建玩法专属 session UI 或按玩法隔离在线行为时，配合 `../orion-gamefeatures/SKILL.md`。
- `Config/Custom/Steam/DefaultEngine.ini` Steam block、`SteamDevAppId`、`SteamAppId`、NetDriver、Web API settings、平台 config 或发布配置，配合 `../orion-project-config/SKILL.md`。
- SteamSockets NetDriver、PacketHandler、RPC/属性复制、listen/dedicated server 网络行为或 packaged multiplayer 验证，配合 `../orion-network-replication-framework/SKILL.md`。
- 好友列表、邀请按钮、在线面板、CommonUI/UMG Widget 或 `GameUI/Online` 修改，配合 `../orion-umg/SKILL.md` 和 `../unreal-commoninput/SKILL.md`。
- 修改 `Content/UI/Online` 的 Friends、Invite、Session Browser Widget Blueprint 或相关 GameUI Online 基类时，同时读取 `../orion-ui-blueprint-framework/SKILL.md`。
- 创建 `BP_SessionComponent` 子类、在线 UI 资产或 GameFeature 内容资产时，配合 `../orion-asset-management/SKILL.md`。
- Steam 打包、Shipping 目标、Dedicated Server、Steam DLL staging、NetDriver 或 AppID 发布检查，配合 `../orion-packaging/SKILL.md`。
