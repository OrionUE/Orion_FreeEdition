...
| 目录 | 职责 |
| --- | --- |
| `Build` | 平台构建资源、应用图标、Pak 黑名单、文件打开顺序、平台 PipelineCaches。Windows 应用图标默认是 `Build/Windows/Application.ico`；替换各平台应用 Logo 时读取 `../unreal-build-platform-icons/SKILL.md`。 |
...
| `GameplayMessageRouter` | 基于 GameplayTag channel 的运行时消息总线、verb message、异步监听和 Blueprint 节点；具体收发、委托对比和网络桥接规则继续读取 `../unreal-gameplay-message-router/SKILL.md`。 |
...
| `OrionSteam` 相关模块 | Steam SDK、OnlineSubsystem、Sockets、WebAPI、shared types。按真实 `.uplugin` 和 `Build.cs` 确认模块名；实现或排查时继续读取 `../unreal-online-steam-framework/SKILL.md`。 |
...
音频、语音聊天、空间音频、MetaSound、AudioMixer、AudioModulation、`CoreAudioSettings`、`DefaultEngine.ini` 音频平台插件或 `Content/Audio` 资产继续读取 `../unreal-audio-framework/SKILL.md`；本架构文档只判断插件组和模块落点。

## 代码落点决策

1. 可复用框架能力：优先确认是否已有 GameCore 抽象；不要默认修改 GameCore。需要框架级变更时先读取 `../unreal-gamecore-framework/SKILL.md` 并等待用户明确允许。
...
8. 跨系统通信：先考虑 `GameplayMessageRouter` 或已有框架消息类型，不要直接做硬引用链；如果消息需要跨 server/client，继续读取 `../unreal-gameplay-message-router/SKILL.md` 和复制相关 Skill。
9. 玩家设置：优先接入 `GameSettings` 和 UI 设置框架。
10. UI 插槽/插件化 UI：优先 `UIExtension`，不要在宿主 HUD 中写死所有 Widget。

## 资产与配置落点

资产命名和目录以 `../unreal-asset-management/SKILL.md` 为准。架构层面只做入口判断：
...
- 视频源文件和 movie 资产：`Content/Movies`；`.bk2`、Bink、启动/加载屏视频、Media Framework 和 movie staging 继续读取 `../unreal-movie-media-framework/SKILL.md`。
...
`DefaultGame.ini`、平台 `Game.ini`、平台 `GameUserSettings.ini`、`DefaultCrypto.ini`、`ProjectPackagingSettings`、AssetManager、CommonInput/CommonUI platform traits 和“用户可以配置哪些参数”继续读取 `../unreal-project-config/SKILL.md`。本架构 Skill 只判断配置属于哪一层，不展开每个字段的语义。

`MoviePlayerSettings`、`StartupMovies`、Bink `.bk2` 配置、`bSkipMovies`、`UFSMovies`、`NonUFSMovies` 和 `Content/Movies` 打包 staging 继续读取 `../unreal-movie-media-framework/SKILL.md`。本架构 Skill 不展开视频格式转换或平台 Media 插件差异。

编辑器扩展、EditorEngine、ToolMenus、DataValidation、GameplayCue 编辑器委托或 `DefaultEditor*.ini` 配置，继续读取 `../unreal-editor-extension-framework/SKILL.md`；架构 Skill 只负责判断落点和模块边界。
