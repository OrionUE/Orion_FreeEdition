...
如果任务涉及创建 Experience、在关卡 WorldSettings 指定 Experience、编辑 `PlayerAccountClass`、`DefaultPawnData`、PawnData、AbilitySet 或 InputAction/InputTag 闭环，先读取 `../../orion-gamemode-experience-framework/SKILL.md`。
...
创建或整理内容目录时，使用 `../../orion-asset-management/SKILL.md`。常见子目录：
...
Experience、ActionSet、PawnData、AbilitySet、InputAction、InputConfig 和关卡 WorldSettings 的资产创建/编辑，使用项目 Experience Toolset；具体函数和可逆验证流程读取 `../../orion-mcp-project-toolsets/SKILL.md`。
...
- 玩家账户、玩家进入/退出状态、玩家 GameplayComponent、玩家出生管理器：配合 `../../orion-player-framework/SKILL.md`。
...
- Feature 内部或多个 feature 之间的 gameplay/UI/通知解耦通信：配合 `../../orion-gameplay-message-router/SKILL.md`。
