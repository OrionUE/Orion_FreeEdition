---
name: orion-network-replication-framework
...
- 判断网络代码应落在核心框架、宿主 Game 模块、GameFeature 插件、Online/Steam 插件、GameUI 还是配置层时，配合 `../orion-framework-architecture/SKILL.md`。
- 触碰 GameCore 只读边界、Experience loading、GameState/PlayerState/PawnData/ASC/PawnExtension/GameplayTagStack 等核心复制链路时，配合 `../orion-gamecore-framework/SKILL.md`。
- Feature 激活/反激活时添加 replicated actor、组件、能力、输入或玩家/队伍系统时，配合 `../orion-gamefeatures/SKILL.md`；需要 graph placement 再配合 `../unreal-replicationgraph/SKILL.md`。
...
- OnlineSubsystem、Steam、Session、Lobby、邀请、SteamSockets NetDriver、Steam target 或发布配置，配合 `../orion-online-steam-framework/SKILL.md`。
...
- 世界物品拾取、丢弃、装备、开箱、容器搜索、`AItemActor`、`UItemInstance`、`UJInventoryComponent`、交互 Ability、交互提示与服务器权威库存修改相关复制，配合 `../orion-inventory-interaction-framework/SKILL.md`。
- GameplayMessageRouter 消息需要跨端、owner-only、server-to-all 或 replicated state bridge 时，配合 `../orion-gameplay-message-router/SKILL.md`。
- 玩家 ID、玩家账户、PlayerInfoComponent、PlayerGameplayComponent、PlayerState/PlayerController 状态复制，配合 `../orion-player-framework/SKILL.md`。
- 角色/PawnData/PawnControl/移动状态/视角/CharacterSpawner/PlayerStart 网络行为，配合 `../orion-character-pawn-framework/SKILL.md`。
- 队伍、TeamInfo、公开/私有队伍数据、友军伤害、AI 敌友态度或按队伍可见性，配合 `../orion-teams-framework/SKILL.md`。
- 语音聊天、麦克风数据、空间语音或音频相关 replicated config/RPC，配合 `../orion-audio-framework/SKILL.md`。
- Dedicated Server、Steam 包、平台 NetDriver 配置、packaged multiplayer 验证或 network cook 问题，配合 `../orion-packaging/SKILL.md` 和 `../orion-project-config/SKILL.md`。
