...
- UI 监听 tag 消息、toast、击杀提示、通知 feed：读取 `../orion-gameplay-message-router/SKILL.md`。
- 玩家可见文案、TextBlock、RichText、按钮文本、设置项名称/描述：读取 `../orion-localization-framework/SKILL.md`，静态文本必须是可收集 `FText`。
- 好友、邀请、Session、Lobby、Steam UI：读取 `../orion-online-steam-framework/SKILL.md`。
- CommonLoadingScreen、启动/运行期加载屏、加载进度、加载屏卡住、shader 编译加载 UI：读取 `../orion-loading-screen-framework/SKILL.md`。
- 编辑器启动或游戏启动 Splash 图片：读取 `../orion-asset-management/SKILL.md`，Splash 是 `Content/Splash` 下的固定图片文件，不是 Widget Blueprint。
- UI 内视频、Bink、Media Texture、菜单背景视频：读取 `../orion-movie-media-framework/SKILL.md`；加载屏视频同时读取 `../orion-loading-screen-framework/SKILL.md`。
- UI 涉及多人状态、owner-only 数据、client/server 消息桥接：读取 `../orion-network-replication-framework/SKILL.md`。
...
| 设置页面 | `UUI_UserSettingScreen`、`UUI_GameWorldSettingScreen` 或 GameSettings 插件基类 | 设置逻辑先读 `orion-game-settings-framework`；UMG 只处理布局、entry、VisualData 和美术 |
| 加载屏 | `UUI_LoadingScreenHost`、`LoadingScreenSubsystem`、`orion-loading-screen-framework` | 通过 `ULoadingScreenManager` 接入，不手动 AddToViewport |
...
修改配置时读取 `../orion-project-config/SKILL.md`，不要把个人编辑器状态写进可发布配置。
...
5. Splash 与 CommonLoadingScreen 加载屏不同：启动 Splash 是项目启动阶段图片；运行期加载屏 Widget、进度条和 shader 编译 UI 仍走 `orion-loading-screen-framework`。
