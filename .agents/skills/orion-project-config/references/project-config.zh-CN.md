...
| `Config/DefaultEditor*.ini` | 编辑器扩展和编辑器用户体验配置。编辑器任务继续读取 `orion-editor-extension-framework`。 |
| `Config/DefaultEditorPerProjectUserSettings.ini` | 编辑器每项目用户设置默认值。UE 5.8 Content Browser 收藏使用 `[/Script/ContentBrowser.ContentBrowserFavoriteProjectSettings] FavoriteItems`，旧 `[ContentBrowser] FavoritePaths` 只作为迁移兼容。 |
...
5. `RendererSettings`、DLSS/Streamline、PSO、`DefaultScalability.ini` 和平台 `Engine.ini` 渲染覆盖继续读取 `orion-rendering-framework`，不要只按普通 `Game` 配置处理。
6. Steam 专用 `OnlineSubsystem`、Steam NetDriver、Steam Web API settings 和 AppID 发布配置默认读取 `Config/Custom/Steam/DefaultEngine.ini` 并继续读取 `orion-online-steam-framework`，不要把真实 AppID 或 Web API key 写进可发布文档。
7. `MoviePlayerSettings`、`StartupMovies`、`BinkMoviePlayerSettings`、`bSkipMovies`、`UFSMovies`、`NonUFSMovies` 和 movie staging 继续读取 `orion-movie-media-framework`，不要把视频路径写成本机绝对路径。
8. `CommonLoadingScreenSettings`、加载屏 Widget、进度条参数、加载屏 hold/heartbeat 或 `LoadingScreenControlBusMix` 继续读取 `orion-loading-screen-framework`。
...
发布前必须提醒用户替换开发/正式 AppID，确认 Dedicated Server 信息，并保护 Web API key。具体字段语义、Session/Invite 行为和验证流程由 `orion-online-steam-framework` 负责。
...
视频设置、渲染 CVar、DLSS、RTX、Scalability 和 PSO 的具体运行时接线由 `orion-rendering-framework` 负责；本手册只负责配置层归属和 `Game`/`GameUserSettings` 默认值。
...
4. 任何会出现在设置页的字段，还要读取 `orion-game-settings-framework`，补齐 registry、Apply/Cancel、VisualData 和保存验证。
...
### Content Browser 收藏插件内容目录无效

现象：在项目默认配置里只写 `[ContentBrowser] FavoritePaths=.../<PluginName>/Content` 后，当前机器内容浏览器 Favorites 没出现目标插件目录。

原因：Content Browser 收藏使用 UE 虚拟内容路径。启用 `CanContainContent=true` 的插件会把自身 `Content` 目录挂载为 `/<PluginName>`，不是 `/<PluginName>/Content`。UE 5.8 的项目收藏最终存储在 `UContentBrowserFavoriteProjectSettings.FavoriteItems`，旧 `FavoritePaths` 只会在 `FavoriteItems` 为空时迁移一次。已经启动过项目的用户如果在 `Saved/Config/WindowsEditor/EditorPerProjectUserSettings.ini` 保存了 `FavoriteItems`，该用户层会覆盖项目默认值。

项目默认修复：只改项目 `Config/DefaultEditorPerProjectUserSettings.ini` 时，同时写新旧格式；新格式用于 UE 5.8，旧格式保留给迁移兼容。

```ini
[/Script/ContentBrowser.ContentBrowserFavoriteProjectSettings]
+FavoriteItems=(Path="/ShooterCore",Alias="",ShortcutCommandName="")
+FavoriteItems=(Path="/ArtFactory",Alias="",ShortcutCommandName="")

[ContentBrowser]
FavoritePaths=/ShooterCore,/ArtFactory
```

限制：项目 `Config` 默认值不会强制覆盖当前用户已保存的 `Saved` 配置，也不能强制打开用户全局 `DisplayFavorites` / `DisplayPluginFolders`。不修改本机用户配置或 `Saved` 的前提下，这个方案适用于新用户、未保存过该设置的用户，或清空/重置用户覆盖后的首次加载。

验证：确认目标插件 `.uplugin` 有 `CanContainContent=true`，目标路径能在 Content Browser 中导航到 `/<PluginName>`；若路径能导航但 Favorites 不显示，检查当前用户是否已有保存过的 `FavoriteItems` 或关闭了 Favorites 区域。
