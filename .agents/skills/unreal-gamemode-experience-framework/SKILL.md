---
name: unreal-gamemode-experience-framework
...
5. 使用 MCP/编辑器自动化前先读取 `../unreal-mcp-project-toolsets/SKILL.md`，调用前用 `list_toolsets` 和 `describe_toolset` 获取真实 schema；没有工具时才补 Toolset。
6. 修改或创建资产前先读取 `../unreal-asset-management/SKILL.md`，遵守蓝图命名和 Content/GameFeature 内容目录规则。
...
- 判断 GameCore 只读边界、宿主 Game 派生层、PawnData、ASC、GameFeature Action 或核心初始化链时，配合 `../unreal-gamecore-framework/SKILL.md`。
- 创建角色蓝图、配置 PawnData 的 `PawnClass`/`InputConfig`/`AbilitySets`/`TagRelationshipMapping`、使用 `BP_CharacterBase` 或角色出生管理时，配合 `../unreal-character-pawn-framework/SKILL.md`。
- Experience 的 `PlayerAccountClass`、玩家账户、玩家信息组件、玩家进入/退出、PlayerGameplayComponent 或 PlayerSpawningManagerComponent 挂载时，配合 `../unreal-player-framework/SKILL.md`；如果涉及玩家/世界存档、LevelMetaDefinition、新游戏/加载游戏或自动保存，继续读取 `../unreal-savegame-framework/SKILL.md`。
- Experience 启用 GameFeature、添加 ActionSet、组件、输入、UI 或能力时，配合 `../unreal-gamefeatures/SKILL.md`。
- Experience 加载状态、过渡关卡、前端 flow 或 Experience/ActionSet 异步工作需要保持加载屏时，配合 `../unreal-loading-screen-framework/SKILL.md`。
- Experience 需要配置队伍数量、TeamCreationComponent、TeamDisplayAsset、按队伍出生、玩家分队或 AI 敌友态度时，配合 `../unreal-teams-framework/SKILL.md`。
- AbilitySet、GA、GamePhaseAbility、ASC 输入或 Ability 输入 Tag 时，配合 `../unreal-gameplayabilities/SKILL.md`。
- InputAction、InputConfig、InputTag、IMC 或改键可见性时，配合 `../unreal-input-framework/SKILL.md` 和 `../unreal-enhancedinput/SKILL.md`。
- 创建或整理 Experience、ActionSet、PawnData、AbilitySet、InputAction、InputConfig、IMC、GA、UI 或 GameFeature 内容资产时，配合 `../unreal-asset-management/SKILL.md`。
- 修改 AssetManager PrimaryAsset 扫描、WorldSettings class、平台配置或 packaging/cook 行为时，配合 `../unreal-project-config/SKILL.md` 和 `../unreal-packaging/SKILL.md`。
- 通过 MCP 创建或编辑 Experience、ActionSet、AbilitySet、InputAction、InputConfig 或关卡默认 Experience 时，配合 `../unreal-mcp-project-toolsets/SKILL.md`。
