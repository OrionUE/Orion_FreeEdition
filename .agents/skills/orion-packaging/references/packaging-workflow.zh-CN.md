...
`Config/DefaultEditorPerProjectUserSettings.ini` 会让编辑器 MCP 默认监听 `127.0.0.1:18765`。如果编辑器开着，Cook 的 `UnrealEditor-Cmd.exe` 也自动启动 MCP，会报 `LogHttpListener: Error: HttpListener unable to bind to 127.0.0.1:18765` 并让 UAT 以 `Error_UnknownCookFailure` 结束。模板和手写 UAT 命令必须通过 Cook 参数禁用 commandlet 自动启动 MCP；不要把项目默认编辑器 MCP 配置改成全局关闭。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .agents\skills\orion-packaging\scripts\install-steam-windows-profiles.ps1 `
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .agents\skills\orion-packaging\scripts\patch-windows-bootstrap-customconfig.ps1 `
...
### 运行时插件默认资源只在 C++ 软路径中引用

现象：打包版启动日志没有 `Fatal error`、`Unhandled Exception` 或 callstack，但日志最后停在运行时插件默认资源缺失，例如：

```text
LogStreaming: Warning: LoadPackage: SkipPackage: /SteamAudio/Materials/Default - The package to load does not exist on disk or in the loader
LogUObjectGlobals: Warning: 未找到Object“Object /SteamAudio/Materials/Default.Default”
```

原因：插件源码里可能用 `FSoftObjectPath` 构造函数默认值或运行时 `TryLoad()` 指向插件 Content。这种引用不一定会被 Cook 自动识别；编辑器能从插件 Content 目录读取，不代表 packaged build 的 UFS / IoStore 里一定包含该资源。

排查：

1. 先确认插件源码或配置中的默认资源路径，例如 `DefaultMeshMaterial`、`DefaultLandscapeMaterial`、`DefaultBSPMaterial`、`Default*Asset` 等。
2. 检查插件 `CanContainContent=true`，并确认对应 `.uasset` 实际存在于插件 `Content` 下。
3. 检查 `ProjectPackagingSettings` 的 `DirectoriesToAlwaysCook` 是否包含该插件资源根，例如 `/SteamAudio/Materials` 或更窄的实际目录。
4. 重新 Cook / Stage 后检查 UFS manifest、IoStore 列表或 `UnrealPak -List`，确认缺失资源进入包内。
5. 打包版 smoke test 重新启动，确认日志不再出现该资源缺失；如果进程仍退出，再继续查后一条有效 Fatal / crash context。

如果 `CrashContext.runtime-xml` 只有 `Unhandled Exception: 0xc06d007e`、`CallStack` 为空，且线程栈主要是 OS、D3D12 或显卡驱动模块，不要忽略游戏日志最后一条可复现的资源缺失；先修 Cook / Stage 内容，再继续追 minidump、驱动或第三方 SDK。

如果 `PCallStack` 或线程栈里出现 `phonon`，说明崩溃已经落到 SteamAudio SDK 侧。`phonon.dll` 在 Win64 上可能 delay-load `OpenCL.dll`、`GPUUtilities.dll`、`TrueAudioNext.dll`；当配置启用 `SceneType=RADEONRAYS` 或 `ReflectionEffectType=TRUEAUDIONEXT` 时，部分没有 OpenCL 运行库或驱动组件不完整的机器可能以 `0xc06d007e` 启动即崩。先把 SteamAudio 配置降到不需要 OpenCL 的路径验证，例如 `SceneType=DEFAULT`、`ReflectionEffectType=CONVOLUTION`；再处理日志尾部 `/SteamAudio/Materials/Default.Default` 这类插件资源 Cook / Stage 缺失。

Steam Common Redistributables 适合安装 VC++、DirectX、OpenAL、PhysX 等 Steamworks 后台列出的通用运行库；不要假设它能自动给所有玩家安装显卡厂商 OpenCL 驱动。OpenCL 依赖应按硬件/驱动能力检测后降级，或提示玩家更新显卡驱动；如果确实需要自定义安装器，使用 Steam install script 维护，但必须确认授权、UAC、重启和失败回退路径。

修复：把必须在启动或早期世界加载阶段 `TryLoad()` 的插件 Content 加入 `DirectoriesToAlwaysCook`。不要只因为插件 Content 在源码树里存在就认为包内也存在；也不要把 Steam Client 未运行、DLSS capability warning 或普通材质 warning 混成同一个崩溃原因。

验证：打包日志或 manifest 能看到对应插件 `.uasset` / `.uexp` 被 Cook / Stage，packaged runtime 日志不再停在该资源缺失；如果日志仍没有 Fatal，需要让测试机器提供 `Saved/Crashes/<CrashGUID>/CrashContext.runtime-xml`、minidump 或更完整的 `-log -abslog` 输出。
