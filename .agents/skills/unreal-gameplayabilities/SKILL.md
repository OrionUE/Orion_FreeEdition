...
- 使用 `UCoreAbilitySystemComponent`、`UCoreGameplayAbility`、`UCoreAbilitySet`、PawnData ability grant、Core ability input flow 或 GameCore ability extension 时，配合 `../orion-gamecore-framework/SKILL.md`。
- AbilitySet 被 PawnData 授予、角色 PawnClass/输入配置影响 GA 激活、或需要创建角色蓝图/PawnData 时，配合 `../orion-character-pawn-framework/SKILL.md`。
- AbilitySet 由 Experience/PawnData/GameFeature 授予、需要创建 Experience、设置 DefaultPawnData、绑定关卡 WorldSettings 或维护 GA/InputTag/InputAction 完整闭环时，配合 `../orion-gamemode-experience-framework/SKILL.md`。
- 能力由输入触发、绑定或映射时，配合 `../orion-input-framework/SKILL.md` 和 `../unreal-enhancedinput/SKILL.md`；UI 按键提示再配合 `../unreal-commoninput/SKILL.md`。
- 交互 Ability、`GameplayAbility_Interact`、`GA_InventoryAbility`、`InteractionAbilityToGrant`、`Ability.Interaction.Activate`、`Event.Inventory.*`、物品拾取/装备/掉落或交互 AbilityTask 时，配合 `../orion-inventory-interaction-framework/SKILL.md`。
- 能力、Attribute、GameplayCue、AbilitySet 或 GamePhase 由 Experience/GameFeature 激活/反激活时，配合 `../orion-gamefeatures/SKILL.md`。
- 伤害执行、目标过滤、锁定、Buff/Debuff 或 GameplayCue 需要敌友判断、友军伤害规则、`FGenericTeamId` 或队伍展示数据时，配合 `../orion-teams-framework/SKILL.md`。
- 创建、命名、移动 GA、GE、AttributeSet、AbilitySet、GameplayCue、InputAction 或 GameFeature 内容资产时，配合 `../orion-asset-management/SKILL.md`。
- Ability 激活失败、AttributeSet 伤害、GameplayCue 参数、GameplayEvent 后续通知、UI 提示或战斗消息需要跨系统广播时，配合 `../orion-gameplay-message-router/SKILL.md`。
- Ability、ASC、AttributeSet、TargetData、GameplayCue、prediction、owner-only 属性或 AbilityActorInfo 涉及多人复制/RPC 时，配合 `../orion-network-replication-framework/SKILL.md`。
