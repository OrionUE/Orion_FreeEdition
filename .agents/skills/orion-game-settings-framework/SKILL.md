---
name: orion-game-settings-framework
...
- 修改 `DefaultEngine.ini` 的 `GameUserSettingsClassName`、`DefaultInput.ini` 的 EnhancedInput user settings、平台 `GameUserSettings.ini` 默认值或配置 section 时，配合 `../orion-project-config/SKILL.md`。
- 设置项属于音频音量、设备、HDR/LDR、空间音频或语音聊天时，配合 `../orion-audio-framework/SKILL.md`。
- 设置项属于渲染、分辨率、画质、DLSS、RTX、帧率、Scalability 或 PSO 启动流程时，配合 `../orion-rendering-framework/SKILL.md`。
- 设置项属于输入、按键重映射、CommonInput 手柄类型、glyph、EnhancedInput mappable profile 或 gamepad 参数时，配合 `../orion-input-framework/SKILL.md`、`../unreal-enhancedinput/SKILL.md` 和 `../unreal-commoninput/SKILL.md`。
- 设置页、Widget Blueprint、CommonUI 层栈、列表 entry、按钮、返回键或 VisualData 资产相关工作，配合 `../orion-umg/SKILL.md`。
- 修改 `Content/UI/Settings` 的设置页 Widget Blueprint、`GameSettingRegistryVisuals`、输入改键 Entry 或世界设置 UI 时，同时读取 `../orion-ui-blueprint-framework/SKILL.md`。
- 语言切换、字幕文本、设置项显示名/说明、Localization 收集/编译/打包，配合 `../orion-localization-framework/SKILL.md`。
- 世界/房间设置与 `WorldOption`、`LevelMeta`、存档保存、前端 phase 或玩法流程相关时，配合 `../orion-savegame-framework/SKILL.md`、`../orion-system-framework/SKILL.md`、`../orion-gamemode-experience-framework/SKILL.md` 和 `../orion-online-steam-framework/SKILL.md`。
- 判断代码落点、GameCore 只读边界、宿主 Game 模块和 GameFeature 插件职责时，配合 `../orion-framework-architecture/SKILL.md` 和 `../orion-gamecore-framework/SKILL.md`。
