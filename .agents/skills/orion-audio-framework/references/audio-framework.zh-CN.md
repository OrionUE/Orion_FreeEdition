...
玩法特定音频跟随对应 GameFeature 插件：`Plugins/GameFeatures/<FeatureName>/Content/Game` 或更具体子目录。资产命名、前缀和覆盖规则以 `orion-asset-management` 为准。
...
SteamAudio 的发布默认值应保持启动安全：`SceneType=DEFAULT`，`ReflectionEffectType=CONVOLUTION`。如果要把 Radeon Rays / TrueAudio Next 做成“可选高级项”，不要只改 `DefaultEngine.ini` 为 GPU 模式；SteamAudio Runtime 模块可能在游戏模块前初始化，游戏模块 `StartupModule` 或设置页创建时再写配置可能已经太晚。推荐做法：

- 在 `FSteamAudioManager::InitializeSteamAudio` 附近读取本地用户设置或 CVar，例如 `SteamAudio.GPUAudioAcceleration`，约定 `-1=Auto`、`0=Disabled`、`1=EnabledWhenSupported`。
- Auto 或 Enabled 时先检测 OpenCL 运行时，例如 Windows 上检查 `OpenCL.dll`，缺失时强制回退 `DEFAULT + CONVOLUTION`，避免 `phonon.dll` 延迟加载 OpenCL 时启动崩溃。
- 即使 OpenCL 运行时存在，也要在 `iplOpenCLDeviceListCreate` / `iplOpenCLDeviceCreate` 后确认 `OpenCLDevice` 非空；如果没有设备，必须在创建 Radeon Rays / TrueAudio Next 前把 `bShouldInitOpenCL`、`bShouldInitRadeonRays`、`bShouldInitTrueAudioNext` 关闭，并把实际模式回退到 `DEFAULT + CONVOLUTION`。
- 玩家可见开关属于本机硬件能力设置，放在 `UOrionSettingsLocal` 和 `GameSettingRegistry_Audio.cpp` 的 Audio 高级分组；保存用户选择后下次启动生效，当前运行中的 SteamAudio 通常不应强行热重建。
- SteamAudio 默认材质如果以插件内容软路径引用，例如 `/SteamAudio/Materials/Default.Default`，打包配置要把 `/SteamAudio/Materials` 加入 always cook，否则打包版可能在启动或加载声学组件时找不到默认材质。
