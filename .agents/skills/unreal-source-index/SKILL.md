...
2. 如果没有索引或索引可能过期，运行 `scripts/generate_unreal_source_index.py` 生成当前机器的相对路径索引；默认缓存到 `<ProjectRoot>/Saved/OrionUE/UnrealEngine/SourceIndex`。
...
```bash
python scripts/generate_unreal_source_index.py --engine-root <EngineRoot>
python scripts/query_unreal_source_index.py EnhancedInput
python scripts/query_unreal_source_index.py ReplicationGraph --kind all
```

如果未传 `--engine-root`，生成脚本会先读取 `<ProjectRoot>/Saved/OrionUE/UnrealEngine/InstallDirectory.txt`，再尝试读取 `UE_ENGINE_ROOT`、`UNREAL_ENGINE_ROOT` 或 `UE_ROOT`。

如果未传 `--out`，默认写入 `<ProjectRoot>/Saved/OrionUE/UnrealEngine/SourceIndex`。生成结果只使用 `Engine/...` 相对路径，不写入本机绝对路径。
...
- 搜索、扫描和索引类缓存统一写入 `<ProjectRoot>/Saved/OrionUE`，不要写入 Skill 目录。
