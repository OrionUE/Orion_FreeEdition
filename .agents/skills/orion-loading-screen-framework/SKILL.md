---
name: orion-loading-screen-framework
...
- 加载屏 Widget Blueprint、CommonUI/UMG 布局、进度条、动画结束回调、内部内容切换，配合 `../orion-umg/SKILL.md`。
- 加载屏 Widget 需要接入当前 `Content/UI` root layout、主题、输入或菜单框架约定时，配合 `../orion-ui-blueprint-framework/SKILL.md`。
- 修改 `[/Script/CommonLoadingScreen.CommonLoadingScreenSettings]`、加载屏 Widget 路径、ZOrder、hold/heartbeat/progress 参数、`CoreAudioSettings.LoadingScreenControlBusMix`，配合 `../orion-project-config/SKILL.md`。
- 加载屏视频、Bink、Media Texture、`Content/Movies` 或 MoviePlayer 启动视频，配合 `../orion-movie-media-framework/SKILL.md`。
- 启动 shader 预编译、PSO、`FShaderPipelineCache`、`FlowStep_TryCompileShaders` 或渲染 benchmark 前端流程，配合 `../orion-rendering-framework/SKILL.md`。
- Experience 加载、GameFeature 激活、ActionSet、Player/Pawn 初始化阻塞加载屏时，配合 `../orion-gamemode-experience-framework/SKILL.md`、`../orion-gamefeatures/SKILL.md`、`../orion-player-framework/SKILL.md` 或 `../orion-character-pawn-framework/SKILL.md`。
- 加载屏显示/隐藏驱动音频 ControlBusMix、背景音遮罩或 MediaSound，配合 `../orion-audio-framework/SKILL.md`。
