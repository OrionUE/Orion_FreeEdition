...
#include "AudioMixEffectsSubsystem.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AudioMixEffectsSubsystem)
...
void UAudioMixEffectsSubsystem::PostInitialize()
{
	...
	if (const UCoreAudioSettings* AudioSettings = GetDefault<UCoreAudioSettings>())
	{
		...
		if (UObject* ObjPath = AudioSettings->LoadingScreenControlBusMix.TryLoad())
		{
			...
			{
				ensureMsgf(SoundControlBusMix, TEXT("Loading Screen Control Bus Mix reference missing from Audio Settings."));
			}
		}
...
void UAudioMixEffectsSubsystem::OnWorldBeginPlay(UWorld& InWorld)
{
	...
	if (const UWorld* World = InWorld.GetWorld())
	{
		...
		if (const UOrionSettingsLocal* SettingsLocal = GetDefault<UOrionSettingsLocal>())
		{
...
bool UAudioMixEffectsSubsystem::DoesSupportWorldType(const EWorldType::Type WorldType) const
{
