...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Game.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Game.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Game.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Game.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Game.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Game.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Game.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
UOrionSettingsLocal::UOrionSettingsLocal()
{
}

UOrionSettingsLocal* UOrionSettingsLocal::Get()
{
...
void UOrionSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UOrionSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UOrionSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UOrionSettingsLocal::GetFrameRateLimit_WhenBackground() const
{
	return FrameRateLimit_WhenBackground;
}

void UOrionSettingsLocal::SetFrameRateLimit_WhenBackground(float NewLimitFPS)
{
	FrameRateLimit_WhenBackground = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UOrionSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UOrionSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
...
int32 UOrionSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UOrionSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UOrionSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UOrionSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return OrionSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UOrionSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return OrionSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UOrionSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UOrionSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UOrionSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EGameFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return OrionSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 4;
...
void UOrionSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGameFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = OrionSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UOrionSettingsLocal::ClampMobileQuality()
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EGameFramePacingMode::MobileStyle)
	{
...
int32 UOrionSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return OrionSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UOrionSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FGameScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UOrionSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UOrionSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = OrionSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UOrionSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	// Mobile resolution quality slider is a normalized value that is leaped between min quality, max quality.
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = OrionSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = OrionSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UOrionSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UOrionSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UOrionSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FGameQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UOrionPlatformEmulationSettings* Settings = GetDefault<UOrionPlatformEmulationSettings>();
...
void UOrionSettingsLocal::UpdateConsoleFramePacing()
{
...
void UOrionSettingsLocal::UpdateDesktopFramePacing()
{
...
void UOrionSettingsLocal::UpdateMobileFramePacing()
{
...
void UOrionSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
...
bool UOrionSettingsLocal::CanRunAutoBenchmark() const
{
	const UGamePlatformSpecificRenderingSettings* PlatformSettings = UGamePlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UOrionSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UOrionSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
	...
	// SetDLSSMode
	if (UGameDLSSSubsystem* DLSSSubsystem = OwningLocalPlayer->GetGameInstance()->GetSubsystem<UGameDLSSSubsystem>())
	{
		// 如果质量低于极高，则禁用DLSS
		if (GetOverallScalabilityLevel() < 3)
		{
			bDLSSUpscaleEnabled = false;
			DLSSSubsystem->RefreshDLSSUpscaleState();
		}
	}
...
void UOrionSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UOrionSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
{
	if (UOrionGameRenderSubsystem* RenderSubsystem = OwningLocalPlayer->GetWorld()->GetSubsystem<UOrionGameRenderSubsystem>())
	{
		if (RenderSubsystem->CanChangeAntiAliasingMethod())
		{
			AntiAliasingMethod = InAntiAliasingMethod;
			DesiredAntiAliasingMethod = InAntiAliasingMethod;
			RenderSubsystem->SetAntiAliasingMethod(InAntiAliasingMethod);
		}
	}
}

void UOrionSettingsLocal::SetDesiredAntiAliasingMethod(EAntiAliasingMethod InAntiAliasingMethod)
{
	DesiredAntiAliasingMethod = InAntiAliasingMethod;
}

void UOrionSettingsLocal::SetRTXEnabled(bool bEnabled)
{
	bRTXEnabled = bEnabled;
}

void UOrionSettingsLocal::SetDLSSUpscaleEnabled(bool bEnabled)
{
	bDLSSUpscaleEnabled = bEnabled;
	if (UGameDLSSSubsystem* DLSSSubsystem = OwningLocalPlayer->GetGameInstance()->GetSubsystem<UGameDLSSSubsystem>())
	{
		DLSSSubsystem->EnableDLSSUpscale(bDLSSUpscaleEnabled);
	}
}

void UOrionSettingsLocal::SetDLSSMode(EGameDLSSMode InDLSSMode)
{
	DLSSMode = InDLSSMode;
	DesiredDLSSMode = InDLSSMode;
}

void UOrionSettingsLocal::SetDesiredDLSSMode(EGameDLSSMode InDLSSMode)
{
	DesiredDLSSMode = InDLSSMode;
}

void UOrionSettingsLocal::SetDLSSFGEnabled(bool bEnabled)
{
	bDLSSFGEnabled = bEnabled;
	if (UGameDLSSSubsystem* DLSSSubsystem = OwningLocalPlayer->GetGameInstance()->GetSubsystem<UGameDLSSSubsystem>())
	{
		DLSSSubsystem->EnableDLSSFG(bDLSSFGEnabled);
	}
}

void UOrionSettingsLocal::SetDLSSFGMode(EStreamlineDLSSGMode InDLSSFGMode)
{
	DLSSFGMode = InDLSSFGMode;
	DesiredDLSSFGMode = InDLSSFGMode;
}

void UOrionSettingsLocal::SetDesiredDLSSFGMode(EStreamlineDLSSGMode InDLSSFGMode)
{
	DesiredDLSSFGMode = InDLSSFGMode;
}
...
void UOrionSettingsLocal::SetOverallVolume(float InVolume)
{
	// Cache the incoming volume value
	OverallVolume = InVolume;

	LoadAndSetVolume(TEXT("Overall"), InVolume);
}

float UOrionSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UOrionSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
	MusicVolume = InVolume;

	LoadAndSetVolume(TEXT("Music"), InVolume);
}

float UOrionSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UOrionSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
	SoundFXVolume = InVolume;
	
	LoadAndSetVolume(TEXT("SoundFX"), InVolume);
}

float UOrionSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UOrionSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
	DialogueVolume = InVolume;

	LoadAndSetVolume(TEXT("Dialogue"), InVolume);
}

float UOrionSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UOrionSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
	VoiceChatVolume = InVolume;

	LoadAndSetVolume(TEXT("VoiceChat"), InVolume);
}

void UOrionSettingsLocal::LoadAndSetVolume(FName SoundClassName, float InVolume)
{
	...
	if (TObjectPtr<USoundControlBus>* ControlBusDblPtr = ControlBusMap.Find(SoundClassName))
	{
		if (USoundControlBus* ControlBusPtr = *ControlBusDblPtr)
		{
...
void UOrionSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UCoreAudioSettings* AudioSettings = GetDefault<UCoreAudioSettings>())
			{
				...
				if (UObject* ObjPath = AudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from Audio Settings."));
					}
				}
...
void UOrionSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UOrionSettingsLocal::SetAudioInputDeviceId(const FString& InAudioInputDeviceId)
{
	AudioInputDeviceId = InAudioInputDeviceId;
	OnAudioInputDeviceChanged.Broadcast(InAudioInputDeviceId);
}

bool UOrionSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

void UOrionSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UOrionSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = OrionSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UOrionSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UOrionSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
...
void UOrionSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UOrionSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
void UOrionSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
	if (ControllerPlatform != InControllerPlatform)
	{
		...
		if (UCommonInputSubsystem* InputSubsystem = UCommonInputSubsystem::Get(OwningLocalPlayer.Get()))
		{
			InputSubsystem->SetGamepadInputType(ControllerPlatform);
		}
	}
...
FName UOrionSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UOrionSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
