...
DECLARE_LOG_CATEGORY_EXTERN(OrionSteamWebAPILog, Log, All);

DECLARE_DYNAMIC_DELEGATE_TwoParams(FOnOrionSteamWebAPICallback, const FString&, data, bool, bWasSuccessful);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnOrionSteamWebAPIAsyncCallback, const FString&, data, bool, bWasSuccessful);
...
{
	OrionSteamWebAPI = 0 UMETA(Hidden),
	Apps,
	Broadcast,
	CheatReporting,
	Community,
...
public:
	FOrionSteamJson()
		: Number(0), Bool(false), Type(ESteamValueType::BOOL)
	{
	}

	FOrionSteamJson(FString Key, FString value)
		: Key(Key), String(value), Number(0), Bool(false), Type(ESteamValueType::STRING)
	{
	}

	FOrionSteamJson(FString Key, double value)
		: Key(Key), String(LexToString(value)), Number(value), Bool(false), Type(ESteamValueType::NUMBER)
	{
	}

	FOrionSteamJson(FString Key, bool value)
		: Key(Key), String(LexToString(value)), Number(0), Bool(value), Type(ESteamValueType::BOOL)
	{
	}

public:
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="OrionSteamWebAPI")
	FString Key;
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="OrionSteamWebAPI")
	FString String;
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category="OrionSteamWebAPI")
...
DECLARE_DYNAMIC_DELEGATE_TwoParams(FOnOrionSteamWebAPIAppListCallback, const TArray<FWebAppsGetAppList>&, Data, bool, bWasSuccessful);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnOrionSteamWebAPIAppListAsyncCallback, const TArray<FWebAppsGetAppList>&, data, bool, bWasSuccessful);
