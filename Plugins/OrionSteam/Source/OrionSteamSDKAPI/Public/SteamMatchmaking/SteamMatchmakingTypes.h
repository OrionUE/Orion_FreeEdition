...
enum class EOrionSteamFindLobbiesState : uint8
{
	Init = 0,
	RequestLobbyList,
	RequestLobbyData,
...
public:
	FFavoritesListChanged()
		: QueryPort(0), ConnectionPort(0), AppID(0), bAdd(false)
	{
	}

#if WITH_ORIONSTEAM
	FFavoritesListChanged(const FavoritesListChanged_t& Data)
		: IP(FIPv4Address(Data.m_nIP).ToString()), QueryPort(Data.m_nQueryPort), ConnectionPort(Data.m_nConnPort), AppID(Data.m_nAppID), bAdd(Data.m_bAdd), SteamID(Data.m_unAccountId)
	{
...
public:
	FLobbyChatMsg()
		: ChatEntryType(ESteamChatEntryType::Invalid), ChatID(0)
	{
	}

#if WITH_ORIONSTEAM
	FLobbyChatMsg(const LobbyChatMsg_t& Data)
		: SteamIDLobby(Data.m_ulSteamIDLobby), SteamIDUser(Data.m_ulSteamIDUser), ChatEntryType(static_cast<ESteamChatEntryType>(Data.m_eChatEntryType)), ChatID(Data.m_iChatID)
	{
	}
#endif
...
public:
	...
	FLobbyChatUpdate(const LobbyChatUpdate_t& Data)
		: SteamIDLobby(Data.m_ulSteamIDLobby), SteamIDUserChanged(Data.m_ulSteamIDUserChanged), SteamIDMakingChange(Data.m_ulSteamIDMakingChange)
	{
...
public:
	...
	FLobbyDataUpdate(const LobbyDataUpdate_t& Data)
		: SteamIDLobby(Data.m_ulSteamIDLobby), SteamIDMember(Data.m_ulSteamIDMember), bSuccess(Data.m_bSuccess > 0)
	{
	}
#endif
...
public:
	FLobbyEnterData()
		: bLocked(false), ChatRoomEnterResponse(ESteamChatRoomEnterResponse::None)
	{
	}

#if WITH_ORIONSTEAM
	FLobbyEnterData(const LobbyEnter_t& Data)
		: SteamIDLobby(Data.m_ulSteamIDLobby), bLocked(Data.m_bLocked), ChatRoomEnterResponse(static_cast<ESteamChatRoomEnterResponse>(Data.m_EChatRoomEnterResponse))
	{
	}
#endif
...
public:
	...
	FLobbyGameCreated(const LobbyGameCreated_t& Data)
		: SteamIDLobby(Data.m_ulSteamIDLobby), SteamIDGameServer(Data.m_ulSteamIDGameServer), IP(FIPv4Address(Data.m_unIP).ToString()), Port(Data.m_usPort)
	{
	}
#endif
...
public:
	...
	FLobbyInviteData(const LobbyInvite_t& Data)
		: SteamIDUser(Data.m_ulSteamIDUser), SteamIDLobby(Data.m_ulSteamIDLobby), GameID(Data.m_ulGameID)
	{
	}
#endif
...
public:
	...
	FLobbyKickedData(const LobbyKicked_t& Data)
		: SteamIDLobby(Data.m_ulSteamIDLobby), SteamIDAdmin(Data.m_ulSteamIDAdmin), bKickedDueToDisconnect(Data.m_bKickedDueToDisconnect > 0)
	{
	}
#endif
...
public:
	...
	FCreateLobbyData(const LobbyCreated_t& Data)
		: Result(_SteamResult(Data.m_eResult)), SteamIDLobby(Data.m_ulSteamIDLobby)
	{
	}
#endif
...
public:
	FJoinLobbyData()
		: bLocked(false), ChatRoomEnterResponse(ESteamChatRoomEnterResponse::None), m_rgfChatPermissions(0)
	{
	}

#if WITH_ORIONSTEAM
	FJoinLobbyData(const LobbyEnter_t& Data)
		: SteamIDLobby(Data.m_ulSteamIDLobby), bLocked(Data.m_bLocked), ChatRoomEnterResponse(static_cast<ESteamChatRoomEnterResponse>(Data.m_EChatRoomEnterResponse)), m_rgfChatPermissions(Data.m_rgfChatPermissions)
	{
	}
#endif
