...
struct ORIONSTEAMSDKAPI_API FSteamP2PSessionState
{
	GENERATED_BODY()
public:
	FSteamP2PSessionState()
		: bConnectionActive(false), bConnecting(false), P2PSessionError(), bUsingRelay(false), BytesQueuedForSend(0), PacketsQueuedForSend(0), RemotePort(0)
	{
	}

#if WITH_ORIONSTEAM
	FSteamP2PSessionState(const P2PSessionState_t& Data)
		: bConnectionActive(Data.m_bConnectionActive > 0), bConnecting(Data.m_bConnecting > 0), P2PSessionError(static_cast<ESteamP2PSessionError>(Data.m_eP2PSessionError)), bUsingRelay(Data.m_bUsingRelay > 0), BytesQueuedForSend(Data.m_nBytesQueuedForSend), PacketsQueuedForSend(Data.m_nPacketsQueuedForSend), RemoteIP(FIPv4Address(Data.m_nRemoteIP).ToString()), RemotePort(Data.m_nRemotePort)
	{
	}
#endif
...
public:
	...
	FP2PSessionConnectFail(const P2PSessionConnectFail_t& Data)
		: SteamIDRemote(Data.m_steamIDRemote), P2PSessionError(static_cast<ESteamP2PSessionError>(Data.m_eP2PSessionError))
	{
	}
#endif
