...
UCLASS()
class ORIONSTEAMSDKAPI_API UOrionSteamUtils : public UOrionSteamInterface
{
	GENERATED_BODY()
public:
	UOrionSteamUtils();
	virtual ~UOrionSteamUtils() override;

	UFUNCTION(BlueprintCallable, BlueprintPure, Category="OrionSteam")
	static UOrionSteamUtils* GetSteamUtils();

public:
	UPROPERTY(BlueprintAssignable, Category="OrionSteam|Utils|Delegates")
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="OrionSteam|Utils")
	 ...
	 * Gets the App ID of the current process.
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure, Category="OrionSteam|Utils|Pure", meta=(DisplayName="Get App ID (Pure)"))
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="OrionSteam|Utils")
	static int32 FilterText(ESteamTextFilteringContext Context, FSteamID SourceSteamID, FString InputMessage, FString& OutFilteredText);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category="OrionSteam|Utils")
	static EOrionSteamIPv6ConnectivityState GetIPv6ConnectivityState(EOrionSteamIPv6ConnectivityProtocol Protocol);
...
#if WITH_ORIONSTEAM
	STEAM_CALLBACK_MANUAL(UOrionSteamUtils, OnCheckFileSignature, CheckFileSignature_t, OnCheckFileSignatureCallback);
	STEAM_CALLBACK_MANUAL(UOrionSteamUtils, OnGamepadTextInputDismissed, GamepadTextInputDismissed_t, OnGamepadTextInputDismissedCallback);
	STEAM_CALLBACK_MANUAL(UOrionSteamUtils, OnLowBatteryPower, LowBatteryPower_t, OnLowBatteryPowerCallback);
	STEAM_CALLBACK_MANUAL(UOrionSteamUtils, OnIPCountry, IPCountry_t, OnIPCountryCallback);
	STEAM_CALLBACK_MANUAL(UOrionSteamUtils, OnSteamShutdown, SteamShutdown_t, OnSteamShutdownCallback);
	STEAM_CALLBACK_MANUAL(UOrionSteamUtils, OnAppResumingFromSuspend, AppResumingFromSuspend_t, OnAppResumingFromSuspendCallback);
	STEAM_CALLBACK_MANUAL(UOrionSteamUtils, OnFloatingGamepadTextInputDismissed, FloatingGamepadTextInputDismissed_t, OnFloatingGamepadTextInputDismissedCallback);
#endif
};
