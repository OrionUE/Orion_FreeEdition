...
struct ORIONSTEAMSDKAPI_API FUGCFileWriteStreamHandle
{
	GENERATED_BODY()
public:
...
public:
	...
	FRemoteStorageFileReadAsyncComplete()
		: Result(ESteamResult::None), Offset(0), Read(0), m_hFileReadAsync(0)
	{
	}
#endif
	...
	FRemoteStorageFileReadAsyncComplete(const RemoteStorageFileReadAsyncComplete_t& Data)
		: Result(_SteamResult(Data.m_eResult)), Offset(Data.m_nOffset), Read(Data.m_cubRead), m_hFileReadAsync(Data.m_hFileReadAsync)
	{
	}
#endif
...
public:
	...
	FRemoteStorageFileShareResult(const RemoteStorageFileShareResult_t& Data)
		: Result(_SteamResult(Data.m_eResult)), File(Data.m_hFile), Filename(Data.m_rgchFilename)
	{
	}
#endif
...
public:
	FRemoteStorageDownloadUGCResult()
		: Result(ESteamResult::None), AppID(0), SizeInBytes(0)
	{
	}

#if WITH_ORIONSTEAM
	FRemoteStorageDownloadUGCResult(const RemoteStorageDownloadUGCResult_t& Data)
		: Result(_SteamResult(Data.m_eResult)), FileHandle(Data.m_hFile), AppID(Data.m_nAppID), SizeInBytes(Data.m_nSizeInBytes), Filename(UTF8_TO_TCHAR(Data.m_pchFileName)), SteamIDOwner(Data.m_ulSteamIDOwner)
	{
	}
#endif
