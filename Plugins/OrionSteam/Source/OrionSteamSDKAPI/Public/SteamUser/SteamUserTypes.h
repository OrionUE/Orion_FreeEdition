...
public:
	FClientGameServerDeny()
		: AppID(0), GameServerPort(0), bSecure(false), Reason(ESteamDenyReason::Invalid)
	{
	}

#if WITH_ORIONSTEAM
	FClientGameServerDeny(const ClientGameServerDeny_t& Data)
		: AppID(Data.m_uAppID), GameServerIP(FIPv4Address(Data.m_unGameServerIP).ToString()), GameServerPort(Data.m_usGameServerPort), bSecure(Data.m_bSecure > 0), Reason(static_cast<ESteamDenyReason>(Data.m_uReason))
	{
	}
#endif
...
public:
	...
	FGetAuthSessionTicketResponse(const GetAuthSessionTicketResponse_t& Data)
		: AuthTicket(Data.m_hAuthTicket), Result(_SteamResult(Data.m_eResult))
	{
	}
#endif
...
public:
	FIPCFailure()
		: FailureType(ESteamFailureType::PipeFail)
	{
	}
...
public:
	FMicroTxnAuthorizationResponse()
		: AppID(0), bAuthorized(false)
	{
	}

#if WITH_ORIONSTEAM
	FMicroTxnAuthorizationResponse(const MicroTxnAuthorizationResponse_t& Data)
		: AppID(Data.m_unAppID), OrderID(LexToString(Data.m_ulOrderID)), bAuthorized(Data.m_bAuthorized > 0)
	{
	}
#endif
...
public:
	FSteamServerConnectFailure()
		: Result(ESteamResult::None), bStillRetrying(false)
	{
	}

#if WITH_ORIONSTEAM
	FSteamServerConnectFailure(const SteamServerConnectFailure_t& Data)
		: Result(_SteamResult(Data.m_eResult)), bStillRetrying(Data.m_bStillRetrying)
	{
	}
#endif
