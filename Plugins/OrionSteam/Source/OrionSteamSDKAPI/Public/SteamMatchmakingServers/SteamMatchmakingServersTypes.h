...
class ORIONSTEAMSDKAPI_API UServerFilter : public UObject
{
	GENERATED_BODY()
public:
	UServerFilter();
...
#if WITH_ORIONSTEAM
	MatchMakingKeyValuePair_t* GetFiltersPtr() const
	{
		return Filters;
	}
#endif
	int32 GetNumFilters() const
	{
		return NumFilters;
	}

private:
	bool CanAddFilter() const
	{
		return NumFilters < MaxFilters;
	}
#if WITH_ORIONSTEAM
	int32 GetKeySize() const
	{
		return sizeof(Filters[NumFilters].m_szKey);
	}
	int32 GetValueSize() const
	{
		return sizeof(Filters[NumFilters].m_szValue);
	}
#endif
private:
	int32 MaxFilters;
...
struct ORIONSTEAMSDKAPI_API FSteamServerAddr
{
	GENERATED_BODY()
public:
	FSteamServerAddr()
...
USTRUCT(BlueprintType)
struct ORIONSTEAMSDKAPI_API FGameServerItem
{
	GENERATED_BODY()
public:
	FGameServerItem()
		: Ping(0), Players(0), MaxPlayers(0), BotPlayers(0), ServerVersion(0), bPassword(false), bSecure(false), m_ulTimeLastPlayed(0), m_nAppID(0), m_bDoNotRefresh(false), bHadSuccessfulResponse(false)
	{
	}

#if WITH_ORIONSTEAM
	FGameServerItem(gameserveritem_t* Data)
		: SteamServerAddr(Data->m_NetAdr, Data->m_steamID), Ping(Data->m_nPing), Players(Data->m_nPlayers), MaxPlayers(Data->m_nMaxPlayers), BotPlayers(Data->m_nBotPlayers), ServerVersion(Data->m_nServerVersion), bPassword(Data->m_bPassword), bSecure(Data->m_bSecure), m_NetAdr(Data->m_NetAdr), m_steamID(Data->m_steamID), m_ulTimeLastPlayed(Data->m_ulTimeLastPlayed), m_nAppID(Data->m_nAppID), m_bDoNotRefresh(Data->m_bDoNotRefresh), bHadSuccessfulResponse(Data->m_bHadSuccessfulResponse)
	{
...
USTRUCT(BlueprintType)
struct ORIONSTEAMSDKAPI_API FGameServerRule
{
	GENERATED_BODY()
public:
	...
	FGameServerRule(const char* InName, const char* InValue)
		: Name(UTF8_TO_TCHAR(InName)), Value(UTF8_TO_TCHAR(InValue))
	{
	}
