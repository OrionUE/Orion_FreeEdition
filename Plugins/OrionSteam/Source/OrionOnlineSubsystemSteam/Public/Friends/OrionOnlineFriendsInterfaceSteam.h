...
#include "OrionOnlineFriendsTypesSteam.h"
...
#if WITH_ORIONSTEAM
class ORIONONLINESUBSYSTEMSTEAM_API FOrionOnlineFriendsSteam : public IOnlineFriends
{
	friend class FOrionOnlineAsyncTaskSteamReadFriendsList;

	struct FSteamFriendsList
	{
		TArray<TSharedRef<FOrionOnlineFriendSteam>> m_Friends;
	};

	PACKAGE_SCOPE : FOrionOnlineFriendsSteam();

public:
	FOrionOnlineFriendsSteam(FOrionOnlineSubsystemSteam* InSteamSubsystem);

	virtual ~FOrionOnlineFriendsSteam() override {
	};
