...
class FOrionWebBrowserViewport;
...
typedef SViewport SOrionBrowserWidget;
#else
typedef SWidget SOrionBrowserWidget;
#endif

class SOrionBrowserView
...
public:
	...
	SLATE_BEGIN_ARGS(SOrionBrowserView)
		...
		, _EnablePaymentRequest(false)
		, _CursorPolicy(EOrionBrowserCursorPolicy::PageControlled)
	...
	{ }
		...
		/** Custom Application part of the user agent for HTTP requests. If not set, WebBrowserInitSettings.ProductVersion will be used. */
		SLATE_ARGUMENT(TOptional<FString>, UserAgentApplication)
		...
		/** Opt in the W3C PaymentRequest API for this browser. */
		SLATE_ARGUMENT(bool, EnablePaymentRequest)

		/** Whether web content may change the Slate/platform mouse cursor. */
		SLATE_ARGUMENT(EOrionBrowserCursorPolicy, CursorPolicy)
		...
		/** Called to allow suppression of the browser context menu. */
	...
	/** Default constructor. */
	ORIONBROWSER_API SOrionBrowserView();
	...
	ORIONBROWSER_API virtual void OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) override;
	ORIONBROWSER_API virtual FReply OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) override;
	ORIONBROWSER_API virtual void OnDragLeave(const FDragDropEvent& DragDropEvent) override;
	ORIONBROWSER_API virtual FReply OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) override;
	 ...
	 */
	ORIONBROWSER_API void Construct(const FArguments& InArgs, const TSharedPtr<IWebBrowserWindow>& InWebBrowserWindow = nullptr);

	ORIONBROWSER_API virtual int32 OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const override;
	 ...
	 */
	ORIONBROWSER_API void LoadURL(FString NewURL);
	...
	*/
	ORIONBROWSER_API void LoadString(FString Contents, FString DummyURL);

	/** Reload the current page. */
	ORIONBROWSER_API void Reload();

	/** Stop loading the page. */
	ORIONBROWSER_API void StopLoad();

	/** Get the current title of the web page. */
	ORIONBROWSER_API FText GetTitleText() const;
	 ...
	 */
	ORIONBROWSER_API FString GetUrl() const;
	 ...
	 */
	ORIONBROWSER_API FText GetAddressBarUrlText() const;

	/** Whether the document finished loading. */
	ORIONBROWSER_API bool IsLoaded() const;

	/** Whether the document is currently being loaded. */
	ORIONBROWSER_API bool IsLoading() const;

	/** Whether the browser widget is done initializing. */
	ORIONBROWSER_API bool IsInitialized() const;

	/** Execute javascript on the current window */
	ORIONBROWSER_API void ExecuteJavascript(const FString& ScriptText);
	 ...
	 */
	ORIONBROWSER_API void GetSource(TFunction<void (const FString&)> Callback) const ;
	 ...
	 */
	ORIONBROWSER_API void BindUObject(const FString& Name, UObject* Object, bool bIsPermanent = true);
	 ...
	 */
	ORIONBROWSER_API void UnbindUObject(const FString& Name, UObject* Object, bool bIsPermanent = true);

	ORIONBROWSER_API void BindAdapter(const TSharedRef<IWebBrowserAdapter>& Adapter);

	ORIONBROWSER_API void UnbindAdapter(const TSharedRef<IWebBrowserAdapter>& Adapter);

	ORIONBROWSER_API void BindInputMethodSystem(ITextInputMethodSystem* TextInputMethodSystem);

	ORIONBROWSER_API void UnbindInputMethodSystem();

	/** Returns true if the browser can navigate backwards. */
	ORIONBROWSER_API bool CanGoBack() const;

	/** Navigate backwards. */
	ORIONBROWSER_API void GoBack();

	/** Returns true if the browser can navigate forwards. */
	ORIONBROWSER_API bool CanGoForward() const;

	/** Navigate forwards. */
	ORIONBROWSER_API void GoForward();

	/** Set parent SWindow for this browser. */
	ORIONBROWSER_API void SetParentWindow(TSharedPtr<SWindow> Window);
	...
	ORIONBROWSER_API void SetBrowserKeyboardFocus();
	...
	ORIONBROWSER_API void CloseBrowser();

	/** Shows floating close button.*/
	ORIONBROWSER_API void ShowFloatingCloseButton(bool bShow, bool bDraggable);

	/** Gets the internal web browser window */
	ORIONBROWSER_API TSharedPtr<IWebBrowserWindow> GetBrowserWindow() const;

private:

	ORIONBROWSER_API void SetupParentWindowHandlers();

	/** Callback for document loading state changes. */
	ORIONBROWSER_API void HandleBrowserWindowDocumentStateChanged(EWebBrowserDocumentState NewState);

	/** Callback to tell slate we want to update the contents of the web view based on changes inside the view. */
	ORIONBROWSER_API void HandleBrowserWindowNeedsRedraw();

	/** Callback for document title changes. */
	ORIONBROWSER_API void HandleTitleChanged(FString NewTitle);

	/** Callback for loaded url changes. */
	ORIONBROWSER_API void HandleUrlChanged(FString NewUrl);

	/** Callback for showing browser tool tips. */
	ORIONBROWSER_API void HandleToolTip(FString ToolTipText);
	 ...
	 */
	ORIONBROWSER_API bool HandleBeforeNavigation(const FString& Url, const FWebNavigationRequest& Request);

	ORIONBROWSER_API bool HandleLoadUrl(const FString& Method, const FString& Url, FString& OutResponse);
	 ...
	 */
	ORIONBROWSER_API bool HandleCreateWindow(const TWeakPtr<IWebBrowserWindow>& NewBrowserWindow, const TWeakPtr<IWebBrowserPopupFeatures>& PopupFeatures);
	 ...
	 */
	ORIONBROWSER_API bool HandleCloseWindow(const TWeakPtr<IWebBrowserWindow>& BrowserWindow);

	/** Callback for handling requests from floating close button */
	ORIONBROWSER_API void HandleFloatingCloseButtonPressed();

	/** Callback for showing dialogs to the user */
	ORIONBROWSER_API EWebBrowserDialogEventResponse HandleShowDialog(const TWeakPtr<IWebBrowserDialog>& DialogParams);

	/** Callback for dismissing any dialogs previously shown  */
	ORIONBROWSER_API void HandleDismissAllDialogs();

	/** Callback for popup window permission */
	ORIONBROWSER_API bool HandleBeforePopup(FString URL, FString Target);

	/** Callback for showing a popup menu */
	ORIONBROWSER_API void HandleShowPopup(const FIntRect& PopupSize);

	/** Callback for hiding the popup menu */
	ORIONBROWSER_API void HandleDismissPopup();

	/** Callback from the popup menu notifiying it has been dismissed */
	ORIONBROWSER_API void HandleMenuDismissed(TSharedRef<IMenu>);
	...
	ORIONBROWSER_API void HandleWindowDeactivated();
	ORIONBROWSER_API void HandleWindowActivated();
	ORIONBROWSER_API bool UnhandledKeyDown(const FKeyEvent& KeyEvent);
	ORIONBROWSER_API bool UnhandledKeyUp(const FKeyEvent& KeyEvent);
	ORIONBROWSER_API bool UnhandledKeyChar(const FCharacterEvent& CharacterEvent);

	ORIONBROWSER_API bool HandleDrag(const FPointerEvent& MouseEvent);
	ORIONBROWSER_API void HandleConsoleMessage(const FString& Message, const FString& Source, int32 Line, EWebBrowserConsoleLogSeverity Serverity);
...
private:
	...
	TSharedPtr<FOrionWebBrowserViewport> BrowserViewport;
	/** Viewport interface for rendering popup menus. */
	TSharedPtr<FOrionWebBrowserViewport>	MenuViewport;
	/** The implementation dependent widget that renders the browser contents. */
	TSharedPtr<SOrionBrowserWidget> BrowserWidget;
...
protected:
	ORIONBROWSER_API bool HandleSuppressContextMenu();
