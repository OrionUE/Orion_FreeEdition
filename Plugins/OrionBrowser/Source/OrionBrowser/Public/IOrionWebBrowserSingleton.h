...
class IOrionWebBrowserWindowFactory
{
public:
	...
	virtual TSharedPtr<IWebBrowserWindow> Create(
		...
		bool bMobileJSReturnInDict = true) = 0;
};
...
struct FCreateBrowserWindowSettings
{
	...
	FString InitialURL;
	FColor BackgroundColor = FColor(255, 255, 255, 255);
	int BrowserFrameRate = 0;
	bool bUseTransparency = false;
	bool bInterceptLoadRequests = true;
	bool bThumbMouseButtonNavigation = false;
	bool bShowErrorMessage = true;
	bool bMobileJSReturnInDict = true;
	/** Whether to enable the W3C PaymentRequest API in this WebView. */
	bool bEnablePaymentRequest = false;
	...
	TOptional<FString> UserAgentApplication;
...
public:
	 ...
	 * Virtual Destructor
	 */
	virtual ~IOrionWebBrowserSingleton() = default;
	...
	virtual TSharedRef<IOrionWebBrowserWindowFactory> GetWebBrowserWindowFactory() const = 0;
