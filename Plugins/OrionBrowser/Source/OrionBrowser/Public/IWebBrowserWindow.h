...
public:
	...
	*/
	...
	/** Sets whether web content can change the Slate/platform cursor. */
	virtual void SetCursorPolicy(EOrionBrowserCursorPolicy InCursorPolicy) {}

	/** Gets whether web content can change the Slate/platform cursor. */
	virtual EOrionBrowserCursorPolicy GetCursorPolicy() const { return EOrionBrowserCursorPolicy::PageControlled; }
	...
	/**
	 * Notify the browser that a drag operation has entered the window.
	 *
	 * @param MyGeometry The Geometry of the browser
	 * @param DragDropEvent Information about the drag event
	 */
	virtual void OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) = 0;

	/**
	 * Notify the browser that a drag operation is moving over the window.
	 *
	 * @param MyGeometry The Geometry of the browser
	 * @param DragDropEvent Information about the drag event
	 * @return FReply indicating how the event was handled
	 */
	virtual FReply OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) = 0;

	/**
	 * Notify the browser that a drag operation has left the window.
	 *
	 * @param DragDropEvent Information about the drag event
	 */
	virtual void OnDragLeave(const FDragDropEvent& DragDropEvent) = 0;

	/**
	 * Notify the browser that a drag operation has been released on it.
	 *
	 * @param MyGeometry The Geometry of the browser
	 * @param DragDropEvent Information about the drag event
	 * @return FReply indicating how the event was handled
	 */
	virtual FReply OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent) = 0;
...
public:
	...
	virtual FOnCheckVisibility& OnCheckVisibility() { return OnCheckVisibilityDelegate; }

	virtual bool CheckVisibility() { return !OnCheckVisibilityDelegate.IsBound() || OnCheckVisibilityDelegate.Execute(); }

protected:

	/** Virtual Destructor. */
	virtual ~IWebBrowserWindow() = default;
