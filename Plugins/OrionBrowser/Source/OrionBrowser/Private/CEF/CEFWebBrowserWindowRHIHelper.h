...
{
public:
	FOrionCEFWebBrowserWindowRHIHelper();
	virtual ~FOrionCEFWebBrowserWindowRHIHelper() = default;

	static bool BUseSupportedRHIRenderer();
	static uint64_t GetRHIAdapterLuid();

public:
	FSlateUpdatableTexture* CreateSlateUpdatableTexture(const FIntPoint& TextureSize);
	void ReleaseSlateUpdatableTexture(FSlateUpdatableTexture* SlateTexture);
	FSlateUpdatableTexture* ResizeSlateUpdatableTexture(FSlateUpdatableTexture* SlateTexture, const FIntPoint& TextureSize);
