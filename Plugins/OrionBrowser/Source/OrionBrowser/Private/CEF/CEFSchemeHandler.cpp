...
{
public:
	FHandlerHeaderSetter(CefRefPtr<CefResponse>& InResponse, int64_t& InContentLength, CefString& InRedirectUrl)
		: Response(InResponse)
...
class FOrionCefSchemeHandler
	: public CefResourceHandler
{
public:
	FOrionCefSchemeHandler(TUniquePtr<IWebBrowserSchemeHandler>&& InHandlerImplementation)
		: HandlerImplementation(MoveTemp(InHandlerImplementation))
	{
	}

	virtual ~FOrionCefSchemeHandler()
	{
	}
...
private:
	...
	IMPLEMENT_REFCOUNTING(FOrionCefSchemeHandler);
};


class FOrionCefSchemeHandlerFactory
	: public CefSchemeHandlerFactory
{
public:

	FOrionCefSchemeHandlerFactory(IWebBrowserSchemeHandlerFactory* InWebBrowserSchemeHandlerFactory)
	...
	{
		return new FOrionCefSchemeHandler(WebBrowserSchemeHandlerFactory->Create(
			WCHAR_TO_TCHAR(Request->GetMethod().ToWString().c_str()),
			WCHAR_TO_TCHAR(Request->GetURL().ToWString().c_str())));
	}
...
private:
	...
	// Include CEF ref counting.
	IMPLEMENT_REFCOUNTING(FOrionCefSchemeHandlerFactory);
};

void FOrionCefSchemeHandlerFactories::AddSchemeHandlerFactory(FString Scheme, FString Domain, IWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
	checkf(WebBrowserSchemeHandlerFactory != nullptr, TEXT("WebBrowserSchemeHandlerFactory must be provided."));
	CefRefPtr<CefSchemeHandlerFactory> Factory = new FOrionCefSchemeHandlerFactory(WebBrowserSchemeHandlerFactory);
...
void FOrionCefSchemeHandlerFactories::RemoveSchemeHandlerFactory(IWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
	checkf(WebBrowserSchemeHandlerFactory != nullptr, TEXT("WebBrowserSchemeHandlerFactory must be provided."));
	SchemeHandlerFactories.RemoveAll([WebBrowserSchemeHandlerFactory](const FFactory& Element)
	{
		return ((FOrionCefSchemeHandlerFactory*)Element.Factory.get())->IsUsing(WebBrowserSchemeHandlerFactory);
	});
}

void FOrionCefSchemeHandlerFactories::RegisterFactoriesWith(CefRefPtr<CefRequestContext>& Context)
{
	if (Context)
	{
...
FOrionCefSchemeHandlerFactories::FFactory::FFactory(FString InScheme, FString InDomain, CefRefPtr<CefSchemeHandlerFactory> InFactory)
