...
static FString GetBindingName(const TSharedPtr<FOrionCEFJSScripting>& Scripting, const FProperty* ValueProperty)
{
...
void FOrionCEFJSStructSerializerBackend::AddNull(const FStructSerializerState& State)
{
...
void FOrionCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, bool Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FOrionCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, int32 Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FOrionCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, double Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FOrionCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, FString Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FOrionCEFJSStructSerializerBackend::Add(const FStructSerializerState& State, UObject* Value)
{
	StackItem& Current = Stack.Top();
	switch (Current.Kind) {
		case StackItem::STYPE_DICTIONARY:
...
void FOrionCEFJSStructSerializerBackend::BeginArray(const FStructSerializerState& State)
{
...
void FOrionCEFJSStructSerializerBackend::BeginStructure(const FStructSerializerState& State)
{
...
void FOrionCEFJSStructSerializerBackend::EndArray(const FStructSerializerState& /*State*/)
{
...
void FOrionCEFJSStructSerializerBackend::EndStructure(const FStructSerializerState& /*State*/)
{
	StackItem Previous = Stack.Pop();
...
void FOrionCEFJSStructSerializerBackend::WriteComment(const FString& Comment)
{
	// Cef values do not support comments
}


void FOrionCEFJSStructSerializerBackend::WriteProperty(const FStructSerializerState& State, int32 ArrayIndex)
{
	// booleans
	...
	{
		GLog->Logf(ELogVerbosity::Warning, TEXT("FOrionCEFJSStructSerializerBackend: Property %s cannot be serialized, because its type (%s) is not supported"), *State.ValueProperty->GetName(), *State.ValueType->GetName());
	}
}
