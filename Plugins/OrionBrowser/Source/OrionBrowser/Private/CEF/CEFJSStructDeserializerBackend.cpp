...
namespace 
{
	...
	void AssignTokenFromContainer(ContainerType Container, KeyType Key,  EStructDeserializerBackendTokens& OutToken, FString& PropertyName, TSharedPtr<IOrionCefContainerWalker>& Retval)
	{
		switch (Container->GetType(Key))
		{
			case VTYPE_NULL:
			...
			case VTYPE_DICTIONARY:
			{
				...
				{
					TSharedPtr<IOrionCefContainerWalker> NewWalker(new FOrionCefDictionaryValueWalker(Retval, Dictionary));
			...
			case VTYPE_LIST:
			{
				TSharedPtr<IOrionCefContainerWalker> NewWalker(new FOrionCefListValueWalker(Retval, Container->GetList(Key)));
				Retval = NewWalker->GetNextToken(OutToken, PropertyName);
				break;
			}
			case VTYPE_BINARY:
	...
	template<typename ContainerType, typename KeyType>
	bool ReadJSFunctionProperty(TSharedPtr<FOrionCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex, CefRefPtr<ContainerType> Container, KeyType Key )
	{
		...
		if ( !StructProperty || StructProperty->Struct != FOrionWebJSFunction::StaticStruct())
		{
			return false;
		}
		...
		FOrionWebJSFunction CallbackObject(Scripting, CallbackID);
	...
	template<typename ContainerType, typename KeyType>
	bool ReadProperty(TSharedPtr<FOrionCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex, CefRefPtr<ContainerType> Container, KeyType Key )
	{
...
TSharedPtr<IOrionCefContainerWalker> FOrionCefListValueWalker::GetNextToken(EStructDeserializerBackendTokens& OutToken, FString& PropertyName)
{
	TSharedPtr<IOrionCefContainerWalker> Retval = SharedThis(this);
	Index++;
	if (Index == -1)
	{
...
bool FOrionCefListValueWalker::ReadProperty(TSharedPtr<FOrionCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex)
{
	return ::ReadProperty(Scripting, Property, Outer, Data, ArrayIndex, List, Index);
}

TSharedPtr<IOrionCefContainerWalker> FOrionCefDictionaryValueWalker::GetNextToken(EStructDeserializerBackendTokens& OutToken, FString& PropertyName)
{
	TSharedPtr<IOrionCefContainerWalker> Retval = SharedThis(this);
	Index++;
	if (Index == -1)
...
bool FOrionCefDictionaryValueWalker::ReadProperty(TSharedPtr<FOrionCEFJSScripting> Scripting, FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex)
{
...
const FString& FOrionCEFJSStructDeserializerBackend::GetCurrentPropertyName() const
{
	return CurrentPropertyName;
}


FString FOrionCEFJSStructDeserializerBackend::GetDebugString() const
{
	return CurrentPropertyName;
}


const FString& FOrionCEFJSStructDeserializerBackend::GetLastErrorMessage() const
{
	return CurrentPropertyName;
}


bool FOrionCEFJSStructDeserializerBackend::GetNextToken( EStructDeserializerBackendTokens& OutToken )
{
	if (Walker.IsValid())
	{
...
bool FOrionCEFJSStructDeserializerBackend::ReadProperty( FProperty* Property, FProperty* Outer, void* Data, int32 ArrayIndex )
{
	return Walker->ReadProperty(Scripting, Property, Outer, Data, ArrayIndex);
}


void FOrionCEFJSStructDeserializerBackend::SkipArray()
{
...
void FOrionCEFJSStructDeserializerBackend::SkipStructure()
{
	EStructDeserializerBackendTokens Token;
	int32 depth = 1;
	while (GetNextToken(Token) && depth > 0)
