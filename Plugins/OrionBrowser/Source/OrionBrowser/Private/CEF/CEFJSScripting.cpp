...
CefRefPtr<CefDictionaryValue> FOrionCEFJSScripting::ConvertStruct(UStruct* TypeInfo, const void* StructPtr)
{
	FOrionCEFJSStructSerializerBackend Backend (SharedThis(this));
...
CefRefPtr<CefDictionaryValue> FOrionCEFJSScripting::ConvertObject(UObject* Object)
{
	CefRefPtr<CefDictionaryValue> Result = CefDictionaryValue::Create();
...
bool FOrionCEFJSScripting::OnProcessMessageReceived(CefRefPtr<CefBrowser> Browser, CefProcessId SourceProcess, CefRefPtr<CefProcessMessage> Message)
{
	bool Result = false;
...
void FOrionCEFJSScripting::SendProcessMessage(CefRefPtr<CefProcessMessage> Message)
{
...
CefRefPtr<CefDictionaryValue> FOrionCEFJSScripting::GetPermanentBindings()
{
	CefRefPtr<CefDictionaryValue> Result = CefDictionaryValue::Create();
...
void FOrionCEFJSScripting::BindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
...
void FOrionCEFJSScripting::UnbindUObject(const FString& Name, UObject* Object, bool bIsPermanent)
{
	const FString ExposedName = GetBindingName(Name, Object);
...
bool FOrionCEFJSScripting::HandleReleaseUObjectMessage(CefRefPtr<CefListValue> MessageArguments)
{
	FGuid ObjectKey;
...
bool FOrionCEFJSScripting::HandleExecuteUObjectMethodMessage(CefRefPtr<CefListValue> MessageArguments)
{
	FGuid ObjectKey;
	// Message arguments are Name, Value, bGlobal
	...
	if (ParamsSize > 0)
	{
		...
		for ( TFieldIterator<FProperty> It(Function); It; ++It )
		{
			...
			if (Param->PropertyFlags & CPF_Parm)
			{
				...
				else
				{
					...
					if (StructProperty && StructProperty->Struct->IsChildOf(FOrionWebJSResponse::StaticStruct()))
					{
						PromiseParam = Param;
					}
					else
		...
		FOrionCEFJSStructDeserializerBackend Backend = FOrionCEFJSStructDeserializerBackend(SharedThis(this), NamedArgs);
		FStructDeserializer::Deserialize(Params, *Function, Backend);
	}

	if (PromiseParam)
	{
		FOrionWebJSResponse* PromisePtr = PromiseParam->ContainerPtrToValuePtr<FOrionWebJSResponse>(Params);
		if (PromisePtr)
		{
			*PromisePtr = FOrionWebJSResponse(SharedThis(this), ResultCallbackId);
		}
	}
	...
	if ( ! PromiseParam ) // If PromiseParam is set, we assume that the UFunction will ensure it is called with the result
	{
		if ( ReturnParam )
		{
			...
			FOrionCEFJSStructSerializerBackend ReturnBackend(SharedThis(this));
	...
	if (Params)
	{
		// Invoke the parameters destructors as needed before freeing the Params memory itself
		for (TFieldIterator<FProperty> It(Function); It && It->HasAnyPropertyFlags(CPF_Parm); ++It)
		{
			if (It->HasAnyPropertyFlags(CPF_NoDestructor) || !It->IsInContainer(Function->GetStructureSize()))
			{
				continue;
			}
...
void FOrionCEFJSScripting::UnbindCefBrowser()
{
	InternalCefBrowser = nullptr;
}

void FOrionCEFJSScripting::InvokeJSErrorResult(FGuid FunctionId, const FString& Error)
{
...
void FOrionCEFJSScripting::InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FOrionWebJSParam Arguments[], bool bIsError)
{
	CefRefPtr<CefListValue> FunctionArguments = CefListValue::Create();
...
void FOrionCEFJSScripting::InvokeJSFunction(FGuid FunctionId, const CefRefPtr<CefListValue>& FunctionArguments, bool bIsError)
{
