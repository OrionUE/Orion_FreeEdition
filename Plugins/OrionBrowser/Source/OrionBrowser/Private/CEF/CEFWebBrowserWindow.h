...
#include "OrionCEFCapturedFrame.h"

#endif

class FBrowserBufferedVideo;
class FOrionCEFBrowserHandler;
...
class FOrionCEFImeHandler;
class ITextInputMethodSystem;
class FOrionCEFWebBrowserWindowRHIHelper;
...
{
	FWebBrowserWindowInfo(CefRefPtr<CefBrowser> InBrowser, CefRefPtr<FOrionCEFBrowserHandler> InHandler)
		: Browser(InBrowser)
		, Handler(InHandler)
	{}
	CefRefPtr<CefBrowser> Browser;
	CefRefPtr<FOrionCEFBrowserHandler> Handler;
};
...
private:
	 ...
	 * @param UserAgentProduct the product part of the user agent string for HTTP requests sent by this window.
	 ...
	 */
	FOrionCEFWebBrowserWindow(CefRefPtr<CefBrowser> Browser, CefRefPtr<FOrionCEFBrowserHandler> Handler, FString Url, TOptional<FString> ContentsToLoad, FString UserAgentProduct, bool bShowErrorMessage, bool bThumbMouseButtonNavigation, bool bUseTransparency, bool bJSBindingToLoweringEnabled, bool bUsingAcceleratedPaint, EOrionBrowserCursorPolicy InCursorPolicy);
...
public:
	/** Virtual Destructor. */
	virtual ~FOrionCEFWebBrowserWindow();
	...
	FString GetUserAgentProduct() const { return UserAgentProduct; }
...
public:
	...
	DECLARE_DERIVED_EVENT(FOrionCEFWebBrowserWindow, IWebBrowserWindow::FOnDocumentStateChanged, FOnDocumentStateChanged);
	virtual FOnDocumentStateChanged& OnDocumentStateChanged() override { return DocumentStateChangedEvent; }

	DECLARE_DERIVED_EVENT(FOrionCEFWebBrowserWindow, IWebBrowserWindow::FOnTitleChanged, FOnTitleChanged);
	virtual FOnTitleChanged& OnTitleChanged() override { return TitleChangedEvent; }

	DECLARE_DERIVED_EVENT(FOrionCEFWebBrowserWindow, IWebBrowserWindow::FOnUrlChanged, FOnUrlChanged);
	virtual FOnUrlChanged& OnUrlChanged() override { return UrlChangedEvent; }

	DECLARE_DERIVED_EVENT(FOrionCEFWebBrowserWindow, IWebBrowserWindow::FOnToolTip, FOnToolTip);
	virtual FOnToolTip& OnToolTip() override { return ToolTipEvent; }

	DECLARE_DERIVED_EVENT(FOrionCEFWebBrowserWindow, IWebBrowserWindow::FOnNeedsRedraw, FOnNeedsRedraw);
	virtual FOnNeedsRedraw& OnNeedsRedraw() override { return NeedsRedrawEvent; }

	virtual FOnBeforeBrowse& OnBeforeBrowse() override { return BeforeBrowseDelegate; }

	virtual FOnLoadUrl& OnLoadUrl() override { return LoadUrlDelegate; }

	virtual FOnCreateWindow& OnCreateWindow() override { return WebBrowserHandler->OnCreateWindow(); }

	virtual FOnCloseWindow& OnCloseWindow() override { return CloseWindowDelegate; }

	virtual FOnFloatingCloseButtonPressed& OnFloatingCloseButtonPressed() override { return FloatingCloseButtonPressedDelegate; }

	virtual FCursorReply OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) override { return CursorPolicy == EOrionBrowserCursorPolicy::GameControlled || Cursor == EMouseCursor::Default ? FCursorReply::Unhandled() : FCursorReply::Cursor(Cursor); }
	virtual void SetCursorPolicy(EOrionBrowserCursorPolicy InCursorPolicy) override;
	virtual EOrionBrowserCursorPolicy GetCursorPolicy() const override { return CursorPolicy; }

	virtual FOnBeforePopupDelegate& OnBeforePopup() override { return WebBrowserHandler->OnBeforePopup(); }
	...
	virtual FOnBeforeResourceLoadDelegate& OnBeforeResourceLoad() override
	{
		...
		{
			WebBrowserHandler->OnBeforeResourceLoad().BindSP(this, &FOrionCEFWebBrowserWindow::HandleOnBeforeResourceLoad);
		}
	...
	virtual FOnResourceLoadCompleteDelegate& OnResourceLoadComplete() override
	{
		...
		{
			WebBrowserHandler->OnResourceLoadComplete().BindSP(this, &FOrionCEFWebBrowserWindow::HandleOnResourceLoadComplete);
		}
	...
	virtual FOnConsoleMessageDelegate& OnConsoleMessage() override
	{
		...
		{
			WebBrowserHandler->OnConsoleMessage().BindSP(this, &FOrionCEFWebBrowserWindow::HandleOnConsoleMessage);
		}

		return ConsoleMessageDelegate;
	}

	DECLARE_DERIVED_EVENT(FOrionCEFWebBrowserWindow, IWebBrowserWindow::FOnShowPopup, FOnShowPopup);
	virtual FOnShowPopup& OnShowPopup() override { return ShowPopupEvent; }

	DECLARE_DERIVED_EVENT(FOrionCEFWebBrowserWindow, IWebBrowserWindow::FOnDismissPopup, FOnDismissPopup);
	virtual FOnDismissPopup& OnDismissPopup() override { return DismissPopupEvent; }

	virtual FOnShowDialog& OnShowDialog() override { return ShowDialogDelegate; }

	virtual FOnDismissAllDialogs& OnDismissAllDialogs() override { return DismissAllDialogsDelegate; }

	virtual FOnSuppressContextMenu& OnSuppressContextMenu() override { return SuppressContextMenuDelgate; }

	virtual FOnDragWindow& OnDragWindow() override { return DragWindowDelegate; }
		
	virtual FOnUnhandledKeyDown& OnUnhandledKeyDown() override { return UnhandledKeyDownDelegate; }

	virtual FOnUnhandledKeyUp& OnUnhandledKeyUp() override { return UnhandledKeyUpDelegate; }

	virtual FOnUnhandledKeyChar& OnUnhandledKeyChar() override { return UnhandledKeyCharDelegate; }
...
private:
	 ...
	 */
	void OnAcceleratedPaint(CefRenderHandler::PaintElementType type, const CefRenderHandler::RectList& DirtyRects, const CefAcceleratedPaintInfo& Info);
	...
	*/
	 ...
	 */
	bool OnFileDialog(CefDialogHandler::FileDialogMode Mode, const CefString& DialogTitle, const CefString& DefaultFilePath, const std::vector<CefString>& AcceptFilters, const std::vector<CefString>& AcceptExtensions, const std::vector<CefString>& AcceptDescriptions, CefRefPtr<CefFileDialogCallback> Callback);
	...
	/**
	 * Called to set the popup menu position and size. For the accelerated paint path,
	 * the size is used to pre-create the popup texture at the correct dimensions.
	 *
		 * @param PopupSize The location and size of the popup widget.
	 */
	...
	/**
	 * Called when the text selection has changed for a text field in a web page, used for IME.
	 * @param Browser The CefBrowser for this window.
...
public:
	...
	*/
	...
	/**
	 * Whether we should use a workaround for this bug -
	 * https://github.com/chromiumembedded/cef/issues/3826
	 */
	void SetCaptureCefFrames(const bool bValue);
	
	/**
	 * Whether the CEF version we're using has a resize bug that sometimes requires requesting new frames after a resize.
	 */
	bool HasResizeBug() const;
	
	/**
	 * Indicates a CEF resize is needed, using heuristics to avoid too many resize requests during an active drag-resize.
	 */
	bool IsCefResizeNeeded() const;

	/**
	 * Requests a resize of the CEF webview to ViewportSize
	 */
	void RequestCefResize();
...
private:
	...
	CefRefPtr<FOrionCEFBrowserHandler> WebBrowserHandler;
	...
	/** The product part for the user agent string of this window. */
	FString UserAgentProduct;
	...
	/** Current DPI scale factor of this window (from the OS). */
	float ViewportDPIScaleFactor;

	/** Current scale factor of this window (from the game viewport). */
	...
	/** Tracks whether web content may change the Slate/platform cursor. */
	EOrionBrowserCursorPolicy CursorPolicy;
	...
	TSharedPtr<FOrionCEFImeHandler> Ime;
#endif
	...
	TUniquePtr<FOrionCEFWebBrowserWindowRHIHelper> RHIRenderHelper;

#if PLATFORM_WINDOWS || PLATFORM_MAC
	bool bInDirectHwndMode;
#endif

	bool bHasCorrectNativeCefBuffer;
	TSharedPtr<FOrionCEFCapturedFrame> CapturedCefFrame;

	size_t NumFramesSinceViewportResize;
};

typedef FOrionCEFWebBrowserWindow FWebBrowserWindow;
