...
class FOrionBrowserHelperRemoteScripting
{

public:

	FOrionBrowserHelperRemoteScripting()
	{}
	...
	{
		if(Value->IsNull()) { return Container->SetNull(Key); }
		else if(Value->IsUndefined()) { return Container->SetNull(Key); } // TODO distinguish between NULL and undef?
		else if(Value->IsBool()) { return Container->SetBool(Key, Value->GetBoolValue()); }
		else if(Value->IsDouble()) { return Container->SetDouble(Key, Value->GetDoubleValue()); }
		else if(Value->IsInt()) { return Container->SetInt(Key, Value->GetIntValue()); }
		else if(Value->IsString()) { return Container->SetString(Key, Value->GetStringValue()); }
		else if(Value->IsUInt()) { return Container->SetDouble(Key, Value->GetUIntValue()); }
		else if(Value->IsDate()) { return Container->SetNull(Key); } // TODO
		else if(Value->IsFunction()) { return Container->SetDictionary(Key, V8FunctionToCef(Parent, Value)); }
		else if(Value->IsArray()) { return Container->SetList(Key, V8ArrayToCef(Value)); }
		else if(Value->IsObject()) { return Container->SetDictionary(Key, V8ObjectToCef(Value)); }
		else { return Container->SetNull(Key); }
	}
...
private:
	...
	FOrionBrowserHelperCallbackRegistry CallbackRegistry;

	TMap<int32,CefRefPtr<CefDictionaryValue>> PermanentBindings;
	TMap<FGuid,CefRefPtr<FOrionBrowserHelperRemoteObject>> RemoteObjects;
	...
	friend class FOrionBrowserHelperRemoteObject;
};
