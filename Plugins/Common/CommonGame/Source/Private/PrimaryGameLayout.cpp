...
/*static*/ UPrimaryGameLayout* UPrimaryGameLayout::GetPrimaryGameLayout(ULocalPlayer* LocalPlayer)
{
	if (LocalPlayer)
	{
		...
		{
			if (UGameUIManagerSubsystemBase* UIManager = GameInstance->GetSubsystem<UGameUIManagerSubsystemBase>())
			{
...
void UPrimaryGameLayout::RemoveAllWidgetFromLayer(FGameplayTag LayerName)
{
	if (UCommonActivatableWidgetContainerBase* RemoveLayer = GetLayerWidget(LayerName))
	{
		RemoveLayer->ClearWidgets();

		for (UCommonActivatableWidget* RemoveWidget : RemoveLayer->GetWidgetList())
		{
			if (RemoveWidget)
			{
				RemoveLayer->RemoveWidget(*RemoveWidget);
			}
		}
	}
}
