...
int32 UCommonGameInstance::AddLocalPlayer(ULocalPlayer* NewPlayer, FPlatformUserId UserId)
{
	...
	if (ReturnVal != INDEX_NONE)
	{
		...
		if (UGameUIManagerSubsystemBase* UIManagerSubsystem = GetSubsystem<UGameUIManagerSubsystemBase>())
		{
...
bool UCommonGameInstance::RemoveLocalPlayer(ULocalPlayer* ExistingPlayer)
{
	...
	if (UGameUIManagerSubsystemBase* UIManagerSubsystem = GetSubsystem<UGameUIManagerSubsystemBase>())
	{
		UIManagerSubsystem->NotifyPlayerDestroyed(Cast<UCommonLocalPlayer>(ExistingPlayer));
	}
