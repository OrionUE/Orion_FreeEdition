...
bool UCommonUIExtensions::IsOwningPlayerUsingMouseAndKeyboard(const UUserWidget* WidgetContextObject)
{
	if (WidgetContextObject)
	{
		if (const UCommonInputSubsystem* InputSubsystem = UCommonInputSubsystem::Get(WidgetContextObject->GetOwningLocalPlayer()))
...
UCommonActivatableWidget* UCommonUIExtensions::PushContentToLayer_ForPlayer(const ULocalPlayer* LocalPlayer, FGameplayTag LayerName, TSubclassOf<UCommonActivatableWidget> WidgetClass)
{
	...
	if (UGameUIManagerSubsystemBase* UIManager = LocalPlayer->GetGameInstance()->GetSubsystem<UGameUIManagerSubsystemBase>())
	{
...
void UCommonUIExtensions::PopContentFromLayer(UCommonActivatableWidget* ActivatableWidget)
{
	...
	{
		if (const UGameUIManagerSubsystemBase* UIManager = LocalPlayer->GetGameInstance()->GetSubsystem<UGameUIManagerSubsystemBase>())
		{
...
void UCommonUIExtensions::PopAllContentFromLayer(const ULocalPlayer* LocalPlayer, FGameplayTag LayerName)
{
	if (const UGameUIManagerSubsystemBase* UIManager = LocalPlayer->GetGameInstance()->GetSubsystem<UGameUIManagerSubsystemBase>())
	{
		if (const UGameUIPolicy* Policy = UIManager->GetCurrentUIPolicy())
		{
			if (UPrimaryGameLayout* RootLayout = Policy->GetRootLayout(CastChecked<UCommonLocalPlayer>(LocalPlayer)))
			{
				RootLayout->RemoveAllWidgetFromLayer(LayerName);
			}
		}
	}
}
