...
TArray<FString> FCommonEditorValidationMessageGatherer::IgnorePatterns;
...
void UEditorValidator::ValidateCheckedOutContent(bool bInteractive, const EDataValidationUsecase InValidationUseCase)
{
	...
	if (AssetRegistryModule.Get().IsLoadingAssets())
	{
		...
		{
			UE_LOG(LogCommonEditorEditor, Display, TEXT("Could not run ValidateCheckedOutContent because asset discovery was still being done."));
		}
		return;
	}
	...
	{
		FCommonEditorValidationMessageGatherer ScopedMessageGatherer;
...
bool UEditorValidator::ValidatePackages(const TArray<FString>& ExistingPackageNames, const TArray<FString>& DeletedPackageNames, int32 MaxPackagesToLoad, TArray<FString>& OutAllWarningsAndErrors, const EDataValidationUsecase InValidationUseCase)
{
	...
	{
		UE_LOG(LogCommonEditorEditor, Display, TEXT("Adding referencers for deleted package %s to be verified"), *DeletedPackageName);
		TArray<FName> PackageReferences;
		AssetRegistry.GetReferencers(FName(*DeletedPackageName), PackageReferences, UE::AssetRegistry::EDependencyCategory::Package);
		for (const FName& Referencer : PackageReferences)
		{
			...
			{
				UE_LOG(LogCommonEditorEditor, Display, TEXT("    Deleted package referencer %s was added to the queue to be verified"), *ReferencerString);
	...
	if (AllPackagesToValidate.Num() > MaxPackagesToLoad)
	{
		...
		UE_LOG(LogCommonEditorEditor, Warning, TEXT("%s"), *WarningMessage);
		...
		if (AssetsToCheck.Num() > 0)
		{
			// Preload all assets to check, so load warnings can be handled separately from validation warnings
			{
				for (const FAssetData& AssetToCheck : AssetsToCheck)
				{
					...
					{
						UE_LOG(LogCommonEditorEditor, Display, TEXT("Preloading %s..."), *AssetToCheck.GetObjectPathString());

						// Start listening for load warnings
						FCommonEditorValidationMessageGatherer ScopedPreloadMessageGatherer;
						...
						if (ScopedPreloadMessageGatherer.GetAllWarningsAndErrors().Num() > 0)
						{
							...
							{
								UE_LOG(LogCommonEditorEditor, Error, TEXT("%s"), *LoadWarning);
							}
...
bool UEditorValidator::ValidateProjectSettings()
{
...

	{
		...
		if (bDeveloperMode)
		{
			...
			UE_LOG(LogCommonEditorEditor, Error, TEXT("%s"), *ErrorMessage);
...
void UEditorValidator::GetChangedAssetsForCode(IAssetRegistry& AssetRegistry, const FString& ChangedHeaderLocalFilename, TArray<FString>& OutChangedPackageNames)
{
	...
	if (Module)
	{
		...
		FString ChangedHeaderRelativeToModule = ChangedHeaderLocalFilename;
		FPaths::MakePathRelativeTo(ChangedHeaderRelativeToModule, *FoundModulePathWithSlash);
		...
		TArray<TWeakObjectPtr<UClass>> ClassList = NativeClassCache.GetClassesInHeader(ChangedHeaderModule, ChangedHeaderRelativeToModule);
		...
		for (TWeakObjectPtr<UClass> ModifiedClassPtr : ClassList)
		{
			...
			if (UClass* ModifiedClass = ModifiedClassPtr.Get())
			{
				...
				UE_LOG(LogCommonEditorEditor, Display, TEXT("Validating Subclasses of %s in %s + %s"), *ModifiedClass->GetName(), *ChangedHeaderModule, *ChangedHeaderRelativeToModule);
				...
				AssetRegistry.EnumerateAssets(Filter, [&BlueprintsDerivedFromNativeModifiedClasses, &bTooManyFiles, &DerivedClassNames, ChangedHeaderModule, ChangedHeaderRelativeToModule](const FAssetData& AssetData)
					{
						...
						if (!AssetData.GetTagValueRef<bool>(FBlueprintTags::IsDataOnly) && !UEditorValidator::IsInUncookedFolder(PackageName))
						{
							...
							if (!ClassFromData.IsEmpty())
							{
								...
								{
									UE_LOG(LogCommonEditorEditor, Display, TEXT("\tAdding %s To Validate"), *PackageName);
									...
									{
										bTooManyFiles = true;
										UE_LOG(LogCommonEditorEditor, Display, TEXT("Too many assets invalidated (Max %d) by change to, %s + %s"), GMaxAssetsChangedByAHeader, *ChangedHeaderModule, *ChangedHeaderRelativeToModule);
