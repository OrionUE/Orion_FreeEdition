...
{
	// Commands do not need this validation step as they loaded the content while running.
...
bool UEditorValidator_Load::GetLoadWarningsAndErrorsForPackage(const FString& PackageName, TArray<FString>& OutWarningsAndErrors)
{
	...
	// Commands shouldn't load the temporary packages since it involves collecting garbage and may destroy objects higher in the callstack. Loading it the one time is probably good enough
	// Also since commands don't use RF_Standalone, this could greatly increase command execution time when loading the same assets over and over
	if (ExistingPackage && !IsRunningCommandlet() && UEditorValidator::ShouldAllowFullValidation() && !ExistingPackage->ContainsMap() && !PackageName.EndsWith(TEXT("_BuiltData")))
	{
		...
		if (bSourceFileExists)
		{
			...
			if (ensure(CopyResult == COPY_OK))
			{
				...
				{
					FCommonEditorValidationMessageGatherer::AddIgnorePatterns(InMemoryReloadLogIgnoreList);
					...
					FCommonEditorValidationMessageGatherer::RemoveIgnorePatterns(InMemoryReloadLogIgnoreList);
				}
				if (LoadedPackage)
				{
