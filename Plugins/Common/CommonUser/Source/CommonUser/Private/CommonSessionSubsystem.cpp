...
FString UCommonSession_HostSessionRequest::ConstructSeamlessTravelURL() const
{
	FString CombinedExtraArgs;
	...
	CombinedExtraArgs += TEXT("?SeamlessTravel");
...
FName UCommonSession_SearchResult::GetServerName() const
{
...
void UCommonSessionSubsystem::GetSessionParam(UCommonSession_HostSessionRequest* Request, FString& OutSessionName, int32& OutMaxPlayers, bool& bUsesPresence, float& OutTimeout)
{
	OutSessionName = Request->SessionName;
	if (OutSessionName.IsEmpty())
	{
		const FName SessionName(NAME_GameSession);
		SessionName.ToString(OutSessionName);
	}
	
	OutMaxPlayers = Request->GetMaxPlayers();
	bUsesPresence = Request->bUseLobbies;
	OutTimeout = 120.f;
}
...
void UCommonSessionSubsystem::SeamlessTravel(APlayerController* HostingPlayer, UCommonSession_HostSessionRequest* Request)
{
	PendingTravelURL = Request->ConstructSeamlessTravelURL();
	GetWorld()->ServerTravel(PendingTravelURL);
}
...
void UCommonSessionSubsystem::JoinSessionInternalOSSv2(ULocalPlayer* LocalPlayer, UCommonSession_SearchResult* Request)
{
	...
	// If the request doesn't have a lobby assume it's a session
	if (!Request->Lobby.IsValid())
	{
		ISessionsPtr Sessions = OnlineServices->GetSessionsInterface();
		check(Sessions);
...
void UCommonSessionSubsystem::OnNetworkFailure(UWorld* InWorld, UNetDriver* NetDriver, ENetworkFailure::Type FailureType, const FString& ErrorString)
{
	K2_OnNetworkFailureEvent.Broadcast(FailureType, ErrorString);
}
