...
public:
	...
	virtual FString ConstructSeamlessTravelURL() const;
...
/** A result object returned from the online system that describes a join-able game session */
UCLASS(BlueprintType)
...
public:
	...
	virtual FName GetServerName() const;
 ...
 * 当网络连接中断，如主机关闭连接
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FCommonSessionOnNetworkFailure_Dynamic, ENetworkFailure::Type, FailureType, const FString&, ErrorString);
...
public:
	...
	*/
	...
	/** 通过SessionRequest获取该Session的参数 */
	UFUNCTION(BlueprintCallable, Category=Session)
	virtual void GetSessionParam(UCommonSession_HostSessionRequest* Request, FString& OutSessionName, int32& OutMaxPlayers, bool& bUsesPresence, float& OutTimeout);
	...
	/** 无缝切换关卡 */
	virtual void SeamlessTravel(APlayerController* HostingPlayer, UCommonSession_HostSessionRequest* Request);

	//////////////////////////////////////////////////////////////////////
	// Events

	/** Native Delegate when a local user has accepted an invitation */
	FCommonSessionOnUserRequestedSession OnUserRequestedSessionEvent;
	/** Event broadcast when a local user has accepted an invitation */
	...
	/** 当网络连接中断 */
	UPROPERTY(BlueprintAssignable, Category = "Events", meta = (DisplayName = "On Network Failure"))
	FCommonSessionOnNetworkFailure_Dynamic K2_OnNetworkFailureEvent;
...
protected:
	...
	*/
	...
	void OnLobbyJoinRequested(const UE::Online::FUILobbyJoinRequested& EventParams);
