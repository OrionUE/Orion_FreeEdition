...
class UGameSettingEditable;
class UGameSettingValueEditable;
...
UCLASS(Abstract, Blueprintable, meta = (Category = "Settings", DisableNativeTick))
class GAMESETTINGS_API UGameSettingListEntrySetting_Editable : public UGameSettingListEntry_Setting
{
	GENERATED_BODY()
...
public:
	...
	void SetValue(const FString& InValue) const;
...
protected:
	...
	void HandleEditableButtonClicked();

	UFUNCTION(BlueprintImplementableEvent, Category="Events", meta=(DisplayName="On Editable Button Clicked"))
	void BP_HandleEditableButtonClicked();
...
protected:
	UPROPERTY()
	TObjectPtr<UGameSettingValueEditable> EditableSetting;

private:	// Bound Widgets
	UPROPERTY(BlueprintReadOnly, meta=(BindWidget, BlueprintProtected=true, AllowPrivateAccess=true))
	TObjectPtr<UGameSettingEditable> Button_Editable;
};
