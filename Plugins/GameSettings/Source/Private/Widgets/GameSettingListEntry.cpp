...
void UGameSettingListEntry_Setting::SetSetting(UGameSetting* InSetting)
{
	...
	SetSettingName(DisplayNameOverride.IsEmpty() ? Setting->GetDisplayName() : DisplayNameOverride);
	if (Text_SettingName)
	{
...
//////////////////////////////////////////////////////////////////////////
// UGameSettingListEntrySetting_Editable
//////////////////////////////////////////////////////////////////////////

void UGameSettingListEntrySetting_Editable::SetSetting(UGameSetting* InSetting)
{
	EditableSetting = Cast<UGameSettingValueEditable>(InSetting);
...
void UGameSettingListEntrySetting_Editable::SetValue(const FString& InValue) const
{
	EditableSetting->SetValueFromString(InValue);
}

void UGameSettingListEntrySetting_Editable::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	
	Button_Editable->OnClicked().AddUObject(this, &ThisClass::HandleEditableButtonClicked);
}

void UGameSettingListEntrySetting_Editable::NativeOnEntryReleased()
{
	Super::NativeOnEntryReleased();

	EditableSetting = nullptr;
}

void UGameSettingListEntrySetting_Editable::OnSettingChanged()
{
	Refresh();
}

void UGameSettingListEntrySetting_Editable::RefreshEditableState(const FGameSettingEditableState& InEditableState)
{
	Super::RefreshEditableState(InEditableState);

	const bool bLocalIsEnabled = InEditableState.IsEnabled();
	Button_Editable->SetIsEnabled(bLocalIsEnabled);
}

void UGameSettingListEntrySetting_Editable::HandleEditConditionChanged(UGameSetting* InSetting)
{
	Super::HandleEditConditionChanged(InSetting);
...
void UGameSettingListEntrySetting_Editable::Refresh()
{
	if (ensure(EditableSetting))
	{
		const FString Value = EditableSetting->GetValueAsString();
		
		Button_Editable->SetValue(Value);
	}
}

void UGameSettingListEntrySetting_Editable::HandleEditableButtonClicked()
{
	BP_HandleEditableButtonClicked();
}

FString UGameSettingListEntrySetting_Editable::GetValueAsString() const
{
	return EditableSetting->GetValueAsString();
}
