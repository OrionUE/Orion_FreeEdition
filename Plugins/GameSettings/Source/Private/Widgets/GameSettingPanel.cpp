...
void UGameSettingPanel::ResetCurrentSelectedItem()
{
	UObject* CurrentSelectedItem = ListView_Settings->GetSelectedItem();
	ListView_Settings->RequestNavigateToItem(CurrentSelectedItem);
	ListView_Settings->SetSelectedItem(CurrentSelectedItem);
}
...
void UGameSettingPanel::HandleSettingItemHoveredChanged(UObject* Item, bool bHovered)
{
	...
	if (bHovered && Setting)
	{
		// 当输入为键盘鼠标时，鼠标移动在该条目上，将该条目设置为当前选择
		if (GetInputSubsystem()->GetCurrentInputType() == ECommonInputType::MouseAndKeyboard)
		{
			ListView_Settings->SetSelectedItem(Item);
		}
