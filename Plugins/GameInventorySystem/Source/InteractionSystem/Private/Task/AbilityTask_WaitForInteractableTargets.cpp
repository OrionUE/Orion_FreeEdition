...
#include "Task/AbilityTask_WaitForInteractableTargets.h"
...
void UAbilityTask_WaitForInteractableTargets::AimWithPlayerController(const AActor* InSourceActor, const FCollisionQueryParams& Params, const FVector& TraceStart, float MaxRange, FVector& OutTraceEnd) const
{
...
bool UAbilityTask_WaitForInteractableTargets::ClipCameraRayToAbilityRange(const FVector& CameraLocation, const FVector& CameraDirection, const FVector& AbilityCenter, float AbilityRange, FVector& ClippedPosition)
{
...
void UAbilityTask_WaitForInteractableTargets::UpdateInteractableOptions(const FInteractionQuery& InteractQuery, const TArray<TScriptInterface<IInteractableTarget>>& InteractableTargets)
{
	...
	for (const auto& InteractiveTarget : InteractableTargets)
	{
		...
		for (FInteractionOption& Option : TempOptions)
		{
			...
			// if there is a handle a target ability system, we're triggering the ability on the target.
	...
	if (bOptionsChanged)
	{
		...
		OnInteractableObjectsChanged.Broadcast(CurrentOptions);
	}
}
