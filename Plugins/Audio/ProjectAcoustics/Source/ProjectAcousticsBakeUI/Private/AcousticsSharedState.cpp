...
namespace
{
	...
	FString NormalizeAcousticsDataDir(FString Path)
	{
	...
	bool IsLegacyAcousticsDataDir(const FString& Path)
	{
		if (Path.IsEmpty())
		{
			return false;
		...
		const FString NormalizedPath = NormalizeAcousticsDataDir(Path);
		TArray<FString> LegacyPaths;
		LegacyPaths.Add(FPaths::Combine(GetProjectAcousticsPluginDir(), TEXT("AcousticsData")));
		LegacyPaths.Add(FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("ProjectAcoustics"), TEXT("AcousticsData")));

		for (const FString& LegacyPath : LegacyPaths)
		{
			if (NormalizedPath.Equals(NormalizeAcousticsDataDir(LegacyPath), ESearchCase::IgnoreCase))
			{
				return true;
			}
		}
	...
	void DestroyAcousticsDebugRenderer(UWorld* EditorWorld, AAcousticsDebugRenderer* DebugRenderer)
	{
		...
		DebugRenderer->SetConfiguration(nullptr);
		if (EditorWorld != nullptr)
		{
			DebugRenderer->Modify();
			EditorWorld->EditorDestroyActor(DebugRenderer, true);
			return;
		}
...
void AcousticsSharedState::InitializeProjectState()
{
	...
	if (config.content_dir.IsEmpty() || IsLegacyAcousticsDataDir(config.content_dir))
	{
		config.content_dir = GetDefaultAcousticsDataDir();
	}
	...
	if (!FPaths::DirectoryExists(config.content_dir))
	{
		...
		PlatformFile.CreateDirectoryTree(*config.content_dir);
	}
...
const FSimulationParameters& AcousticsSharedState::GetDefaultSimulationParameters()
{
	return m_PythonBridge->GetDefaultSimulationParameters();
}
...
const TritonSimulationParameters AcousticsSharedState::GetTritonSimulationParameters()
{
	...
	params.SimulationFrequency = simulation_config.max_frequency;
	...
	auto tritonVectorFromUnreal = [](const FVector& uv)
	{
		return ATKVectorD{
			...
			AcousticsUtils::UnrealValToTriton(uv.Z)};
	};
	...
	params.PerProbeSimulationRegion = tritonBoxFromUnreal(simulation_config.simulation_region);

	params.ProbeSpacing.MinHorizontalSpacing =
		AcousticsUtils::UnrealValToTriton(simulation_config.probe_spacing.horizontal_spacing_min);
	params.ProbeSpacing.MaxHorizontalSpacing =
		AcousticsUtils::UnrealValToTriton(simulation_config.probe_spacing.horizontal_spacing_max);
	params.ProbeSpacing.VerticalSpacing =
		AcousticsUtils::UnrealValToTriton(simulation_config.probe_spacing.vertical_spacing);
	params.ProbeSpacing.MinHeightAboveGround =
		AcousticsUtils::UnrealValToTriton(simulation_config.probe_spacing.min_height_above_ground);
...
// Whether a prebake is in progress, done, or failed
bool AcousticsSharedState::IsPrebakeActive()
{
	// Prebake is active if a sim config exists
	return GetSimulationConfiguration() != nullptr;
}
...
void AcousticsSharedState::SetSimulationConfiguration(TUniquePtr<AcousticsSimulationConfiguration> config)
{
	...
	{
		// Don't need the debug renderer anymore. Remove every renderer that may still hold stale probe cache.
		if (EditorWorld != nullptr)
		{
			TArray<AAcousticsDebugRenderer*> DebugRenderersToDestroy;
			for (TActorIterator<AAcousticsDebugRenderer> ActorIterator(EditorWorld); ActorIterator; ++ActorIterator)
			{
				DebugRenderersToDestroy.Add(*ActorIterator);
			}

			for (AAcousticsDebugRenderer* DebugRenderer : DebugRenderersToDestroy)
			{
				DestroyAcousticsDebugRenderer(EditorWorld, DebugRenderer);
			}
		}
		else if (m_DebugRenderer.IsValid())
		{
			DestroyAcousticsDebugRenderer(nullptr, m_DebugRenderer.Get());
		}
		m_DebugRenderer.Reset();
		m_SimulationConfiguration.Reset();
	...
	AAcousticsDebugRenderer* DebugRendererToUse =
		m_DebugRenderer.IsValid() && m_DebugRenderer->GetWorld() == EditorWorld ? m_DebugRenderer.Get() : nullptr;
	TArray<AAcousticsDebugRenderer*> DebugRenderersToDestroy;
	for (TActorIterator<AAcousticsDebugRenderer> ActorIterator(EditorWorld); ActorIterator; ++ActorIterator)
	{
		...
		if (DebugRendererToUse == nullptr)
		{
			DebugRendererToUse = DebugRenderer;
			continue;
		}

		if (DebugRenderer != DebugRendererToUse)
		{
			DebugRenderersToDestroy.Add(DebugRenderer);
		}
	}
	...
	if (DebugRendererToUse == nullptr)
	{
		...
		DebugRendererToUse = Cast<AAcousticsDebugRenderer>(
			GEditor->AddActor(TargetLevel, AAcousticsDebugRenderer::StaticClass(), FTransform::Identity));
	}

	m_DebugRenderer = TWeakObjectPtr<AAcousticsDebugRenderer>(DebugRendererToUse);
