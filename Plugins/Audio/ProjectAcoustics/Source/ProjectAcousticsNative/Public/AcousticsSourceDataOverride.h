...
class USoundEffectAcousticsSpatialReverbInputPreset;
class USoundSubmix;
...
private:
	...
	void InitializeSpatialReverbInputCapture(const FAudioPluginInitializationParams& InitializationParams);
	void AddSpatialReverbInputCaptureSend(const uint32 SourceId, FWaveInstance* InOutWaveInstance);
...
private:
	...
	TArray<TStrongObjectPtr<USoundSubmix>> m_SpatialReverbInputSubmixes;
	TArray<TStrongObjectPtr<USoundEffectAcousticsSpatialReverbInputPreset>> m_SpatialReverbInputPresets;
	TArray<FSoundSubmixSendInfo> m_SpatialReverbInputSubmixSends;
};
