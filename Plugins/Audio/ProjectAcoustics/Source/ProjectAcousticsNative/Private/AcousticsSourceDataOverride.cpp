...
void FAcousticsSourceDataOverride::Initialize(const FAudioPluginInitializationParams InitializationParams)
{
	...
	if (m_ReverbType == EAcousticsReverbType::SpatialReverb)
	{
		...
		InitializeSpatialReverbInputCapture(InitializationParams);
...
void FAcousticsSourceDataOverride::InitializeSpatialReverbInputCapture(
	const FAudioPluginInitializationParams& InitializationParams)
{
	const int32 NumSources = static_cast<int32>(InitializationParams.NumSources);
	m_SpatialReverbInputSubmixes.SetNum(NumSources);
	m_SpatialReverbInputPresets.SetNum(NumSources);
	m_SpatialReverbInputSubmixSends.SetNum(NumSources);

	for (int32 SourceIndex = 0; SourceIndex < NumSources; ++SourceIndex)
	{
		auto* Preset = NewObject<USoundEffectAcousticsSpatialReverbInputPreset>();
		Preset->SourceDataOverridePtr = this;
		Preset->SourceId = SourceIndex;

		auto* Submix = NewObject<USoundSubmix>();
		Submix->SubmixEffectChain.Add(Preset);

		FSoundSubmixSendInfo& SendInfo = m_SpatialReverbInputSubmixSends[SourceIndex];
		SendInfo.SoundSubmix = Submix;
		SendInfo.SendLevelControlMethod = ESendLevelControlMethod::Manual;
		SendInfo.SendLevel = 1.0f;
		SendInfo.SendStage = ESubmixSendStage::PreDistanceAttenuation;

		m_SpatialReverbInputPresets[SourceIndex].Reset(Preset);
		m_SpatialReverbInputSubmixes[SourceIndex].Reset(Submix);
	}
}

void FAcousticsSourceDataOverride::AddSpatialReverbInputCaptureSend(
	const uint32 SourceId, FWaveInstance* InOutWaveInstance)
{
	if (!InOutWaveInstance || !m_SpatialReverbInputSubmixSends.IsValidIndex(SourceId))
	{
		return;
	}

	FSoundSubmixSendInfo& SendInfo = m_SpatialReverbInputSubmixSends[SourceId];
	if (SendInfo.SoundSubmix != nullptr)
	{
		const USoundSubmixBase* CaptureSubmix = SendInfo.SoundSubmix;
		const bool bAlreadyAdded = InOutWaveInstance->SoundSubmixSends.ContainsByPredicate(
			[CaptureSubmix](const FSoundSubmixSendInfo& ExistingSendInfo)
			{
				return ExistingSendInfo.SoundSubmix == CaptureSubmix;
			});
		if (bAlreadyAdded)
		{
			return;
		...
		InOutWaveInstance->bEnableSubmixSends = true;
		InOutWaveInstance->SoundSubmixSends.Add(SendInfo);
	}
}
	...
	const float occlusionDbActual, const AcousticsObjectParams& objectParams, FWaveInstance* InOutWaveInstance)
{
	...
	if (IsSpatialReverbInitialized())
	{
		AddSpatialReverbInputCaptureSend(SourceId, InOutWaveInstance);
