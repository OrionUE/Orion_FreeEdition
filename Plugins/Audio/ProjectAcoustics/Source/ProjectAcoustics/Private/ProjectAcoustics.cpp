...
int32 c_FallbackToNearestProbe = 0;
static FAutoConsoleVariableRef CVarAcousticsFallbackToNearestProbe(
	TEXT("PA.FallbackToNearestProbe"), c_FallbackToNearestProbe,
	TEXT("Retry failed acoustic queries with nearest-probe interpolation. This is diagnostic-only for large maps.\n")
		TEXT("0 disables fallback, 1 enables fallback.\n"),
	ECVF_Default);
...
static constexpr uint32 c_AcousticsQueryThreadStackSizeBytes = 4 * 1024 * 1024;
	...
	, m_NumRunningTasks(0)
{
	...
	// Triton probe decompression can exceed UE's queued-thread default stack size.
	m_ThreadPool = FQueuedThreadPool::Allocate();
	if (!m_ThreadPool->Create(1, c_AcousticsQueryThreadStackSizeBytes, TPri_Normal, TEXT("ProjectAcousticsQueryThreadPool")))
	{
		UE_LOG(LogAcousticsRuntime, Error, TEXT("Failed to create ProjectAcoustics query thread pool."));
	}
}
	...
	TritonDynamicOpeningInfo& outOpeningInfo, const InterpolationConfig& interpConfig, TritonRuntime::QueryDebugInfo* outDebugInfo /* = nullptr */)
{
	...
	auto listener = AcousticsUtils::ToTritonVectorDouble(WorldPositionToTriton(listenerLocation));
	const TritonDynamicOpeningInfo OriginalOpeningInfo = outOpeningInfo;
	...
	if (!acousticParamsValid && c_FallbackToNearestProbe != 0 &&
		interpConfig.Resolver != InterpolationConfig::DisambiguationMode::Nearest)
	{
		TritonAcousticParameters FallbackParams = {};
		TritonDynamicOpeningInfo FallbackOpeningInfo = OriginalOpeningInfo;
		InterpolationConfig FallbackInterpConfig(InterpolationConfig::DisambiguationMode::Nearest);

#if !UE_BUILD_SHIPPING
		TritonRuntime::QueryDebugInfo FallbackDebugInfo;
#endif

		bool fallbackParamsValid = false;
		{
			SCOPE_CYCLE_COUNTER(STAT_Acoustics_Query);

#if !UE_BUILD_SHIPPING
			fallbackParamsValid = GetTritonDebugInstance()->QueryAcoustics(
				source, listener, FallbackParams, FallbackOpeningInfo, &FallbackInterpConfig, &FallbackDebugInfo);
#else
			fallbackParamsValid = m_Triton->QueryAcoustics(source, listener, FallbackParams, FallbackOpeningInfo, &FallbackInterpConfig);
#endif
		}

		if (fallbackParamsValid)
		{
			params = FallbackParams;
			outOpeningInfo = FallbackOpeningInfo;

#if !UE_BUILD_SHIPPING
			if (outDebugInfo != nullptr)
			{
				*outDebugInfo = FallbackDebugInfo;
			}
#endif

			UE_LOG(LogAcousticsRuntime, Verbose, TEXT("Project Acoustics query fallback to nearest probe succeeded."));
			return true;
		}
	}
