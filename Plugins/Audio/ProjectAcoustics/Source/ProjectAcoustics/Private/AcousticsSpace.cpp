...
#if !UE_BUILD_SHIPPING
	EnsureDebugRenderingRegistered();
...
void AAcousticsSpace::Tick(float deltaSeconds)
{
	...
	if (GetWorld()->IsGameWorld())
	{
		...
		// Update distances
		bool bShouldUpdateDistances = UpdateDistances;
#if !UE_BUILD_SHIPPING
		bShouldUpdateDistances = bShouldUpdateDistances || CVarAcousticsDrawDistances.GetValueOnGameThread() > 0;
#endif // !UE_BUILD_SHIPPING
		if (bShouldUpdateDistances)
		{
...
#if !UE_BUILD_SHIPPING
bool AAcousticsSpace::ShouldRenderDebugOverlays() const
{
	return DrawStats ||
		DrawVoxels ||
		DrawProbes ||
		...
		CVarAcousticsShowStats.GetValueOnGameThread() > 0 ||
		CVarAcousticsDrawVoxels.GetValueOnGameThread() > 0 ||
		CVarAcousticsDrawProbes.GetValueOnGameThread() > 0 ||
		CVarAcousticsDrawDistances.GetValueOnGameThread() > 0 ||
		CVarAcousticsShowAllSourceParameters.GetValueOnGameThread() > 0;
}

void AAcousticsSpace::EnsureDebugRenderingRegistered()
{
	if (!ShouldRenderDebugOverlays())
	{
		return;
	}
...
void AAcousticsSpace::PostRenderFor(APlayerController* PC, UCanvas* Canvas, FVector CameraPosition, FVector CameraDir)
{
	...
	m_Acoustics->DebugRender(
		...
		DrawDistances || CVarAcousticsDrawDistances.GetValueOnGameThread() > 0,
